...
if (isset($_GET['load'])) {
    ...
    $tabela = "<div class='table-responsive'>
                                                    <table id='lang_opt10' class='table table-bordered table-striped'>
                                                        <thead align='center'>
                                                        <tr>
                                                            <th><strong><h4>Cód. Formando</h4></strong></th>
                                                            <th><strong><h4>Nome</h4></strong></th>
                                                            <th><strong><h4>Status</h4></strong></th>
                                                            <th><strong><h4>Ação</h4></strong></th>
                                                        </tr>
                                                        </thead>
                                                        <tbody>";
    while ($vetor = mysqli_fetch_array($result_formandos)) {
        $perfil = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM meuperfilfotografico WHERE id_formando = '$vetor[id_formando]'"));
        $lock = 0;
        if (!empty($perfil['id_formando'])) {
            $status = "Em Aberto";
            $lock = 1;
        } elseif ($perfil['status'] == 1) {
            $status = "Preenchido";
        } else {
            $status = "Em Aberto";
        }
        $tabela .= "                                                   <tr>
                                                                <td align='center'>" . $turma['ncontrato'] . "-" . $vetor['id_cadastro'] . "</td>
                                                                <td align='center'>" . $vetor['nome'] . "</td>
                                                                <td>" . $status . "</td>
                                                                <td align='center'>";
            if ($lock == 0) {
                $tabela .= "<a href='fotografia_perfilformando.php?id_formando=" . $vetor['id_formando'] . "' target='_blank'>
                                                                    <button type='button'
                                                                            class='btn btn-success btn-xs' style='padding: 3px;height: 30px;vertical-align: middle'>
                                                                        <i class='mdi mdi-24px mdi-eye' style='position: relative;top: -6px'></i>
                                                                        </button>
                                                                        </a>";
            }
             $tabela .= "</td>
            ...
            <script type="text/javascript">
                $(document).ready(function () {
                    $('#turmas').change(function () {
                        $('#formando').load('fotografia_perfilfotografico.php?load=sim&id_turma=' + $('#turmas').val(), function () {
                            init_data_Table();
                        });
                    });
                });
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <div name="id_formando" id="formando">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
