...
<!--                                  <h4 class="card-title">Processos</h4>-->
																...
																}else { ?><a href="cadastrar_juridico.php">
                                  ...
                                  <div class="table-responsive">
                                      <table id="lang_opt" class="table table-striped table-bordered display"
                                          ...
                                          <tr>
                                              <th>N° Processo</th>
                                              <th>Partes</th>
                                              <th>Qualificação de Partes</th>
                                              <th width="13%">Ação</th>
                                          </tr>
                                          </thead>
                                          <tbody>
																					<?php
																					$sql_atual = mysqli_query($con, "select * from juridico where status = '1' and formapag = '2' order by data ASC");
																					while ($vetor = mysqli_fetch_array($sql_atual)) {
																						if ($vetor['tipoqualificar'] == 1) {
																							$sql_qualificar = mysqli_query($con, "select * from formandos where id_formando = '$vetor[id_responsavel]'");
																							$vetor_qualificar = mysqli_fetch_array($sql_qualificar);
																							$responsavel = $vetor_qualificar['nome'];
																						}
																						if ($vetor['tipoqualificar'] == 2) {
																							$sql_qualificar = mysqli_query($con, "select * from formandos where id_formando = '$vetor[id_responsavel]'");
																							$vetor_qualificar = mysqli_fetch_array($sql_qualificar);
																							$responsavel = $vetor_qualificar['nome'];
																						}
																						if ($vetor['tipoqualificar'] == 3) {
																							$sql_qualificar = mysqli_query($con, "select * from colaboradores where id_colaborador = '$vetor[id_responsavel]'");
																							$vetor_qualificar = mysqli_fetch_array($sql_qualificar);
																							$responsavel = $vetor_qualificar['nome'];
																						}
																						if ($vetor['tipoqualificar'] == 4) {
																							$responsavel = $vetor['responsaveloutro'];
																						}
																						?>
                                              <tr>
                                                  <td><?php echo $vetor['id_juridico']; ?></td>
                                                  <td><?php if ($vetor['tipoparte'] == 1) {
																											echo "Autor";
																										}
																										if ($vetor['tipoparte'] == 1) {
																											echo "Réu";
																										} ?></td>
                                                  <td><?php echo $responsavel; ?></td>
                                                  <td><a class="fancybox fancybox.ajax"
                                                         href="alterarjuridico.php?id=<?php echo $vetor['id_juridico']; ?>"
                                                         target="_blank">
                                                          <button type="button" class="btn btn-success mesmo-tamanho"
                                                                 ...
                                                                 href="confexcluirjuridico.php?id=<?php echo $vetor['id_juridico']; ?>">
