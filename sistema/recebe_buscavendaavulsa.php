...
function formatarCPF_CNPJ($campo, $formatado = true){
  ...
  $sql_produto = mysqli_query($con, "select * from venda_avulsa where id_turma = '$id_turma' and id_formando = '$id_formando'");
      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            ...
            <?php if($vetor_permissao['cadastro'] == 1) { } else { ?><a href="cadastrovendaavulsa.php"><button class="btn btn-primary"  style="    float: left;">Cadastrar Novo Produto</button></a>
            <br>
            <br>
           	<br><?php } ?>
              <table id="example1" class="table table-bordered table-striped">
                ...
                $sql_forma = mysqli_query($con, "select * from formaspag where id_forma = '$vetor[formapag]'");
                ...
                <tr>
                  ...
                  <td><?php echo $num = number_format($vetor['valorentrada'],2,',','.'); ?></td>
                  <td><?php $valorapagar = $vetor['valor'] - $vetor['valorentrada']; echo $num = number_format($valorapagar,2,',','.'); ?></td>
                  <td><?php echo $vetor_forma['nome']; ?></td>
                  <td><?php if($vetor['status'] == 1) { echo "Em Aberto"; } if($vetor['status'] == 2) { echo "Vendido"; } ?></td>
                  <td><?php if($vetor['status'] == 1) { ?><a href="alterarvendaavulsa.php?id=<?php echo $vetor['id_avulsa']; ?>" target="_blank"><button type="button" class="btn btn-info mesmo-tamanho" title="Ver ou Alterar Cadastro"><i class="fa fa-edit"></i></button></a> <?php if($vetor_permissao['exclusao'] == 1) { } else { ?><a href="confexcluirvendaavulsa.php?id=<?php echo $vetor['id_avulsa']; ?>" ><button type="button" class="btn btn-danger mesmo-tamanho" title="Excluir Cadastro"><i class="fa fa-close"></i></button></a><?php } } ?></td> 
                </tr>
                <?php } ?>
                </tbody>
                <tfoot>
