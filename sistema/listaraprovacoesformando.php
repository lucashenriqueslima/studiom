...
function formatarCPF_CNPJ($campo, $formatado = true){
  ...
  if($id == 10) {
    ...
    $sql_atual = mysqli_query($con, "select * from meu_album where id_formando = '$id_formando' order by id_meualbum DESC");

  } else {

    $sql_atual = mysqli_query($con, "select * from minhas_aprovacoes where id_formando = '$id_formando' order by id_minhaaprovacao DESC");
      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            ...
            <?php if($vetor_permissao['cadastro'] == 1) { } else { if($id == 10) { ?><a href="cadastroalbumformando1.php?id=<?php echo $id; ?>&id_formando=<?php echo $id_formando; ?>"><button class="btn btn-primary"  style="    float: left;">Cadastrar Novo Álbum</button></a><?php } else { ?> <a href="cadastroaprovacaoformando.php?id=<?php echo $id; ?>&id_formando=<?php echo $id_formando; ?>"><button class="btn btn-primary"  style="    float: left;">Cadastrar Nova Aprovação</button></a> <?php } ?>
            <br>
            <br>
           	<br><?php } ?>
              <table id="example1" class="table table-bordered table-striped">
                ...
                $vetor_curso_inicio = mysqli_fetch_array($sql_curso_inicio);
				        ...
				        ?>
                <tr>
                  <td><?php echo $vetor_contrato['ncontrato']; ?> - <?php echo $vetor_curso_inicio['nome']; ?> <?php echo $vetor_contrato['ano']; ?> <?php echo $vetor_instituicao_inicio['nome']; ?></td>
                  ...
                  <td><?php if($id == 10) { ?><a href="alteraralbumformando.php?id=<?php echo $vetor['id_meualbum']; ?>" target="_blank"><button type="button" class="btn btn-info mesmo-tamanho" title="Ver ou Alterar Cadastro"><i class="fa fa-edit"></i></button></a> <?php } else { ?> <a href="alteraraprovacaoformando.php?id=<?php echo $vetor['id_minhaaprovacao']; ?>" target="_blank"><button type="button" class="btn btn-info mesmo-tamanho" title="Ver ou Alterar Cadastro"><i class="fa fa-edit"></i></button></a> <?php } if($vetor_permissao['exclusao'] == 1) { } else { if($id == 10) { ?><a href="confexcluiralbumformando1.php?id=<?php echo $vetor['id_meualbum']; ?>&id1=<?php echo $id; ?>&id_formando=<?php echo $id_formando; ?>" ><button type="button" class="btn btn-danger mesmo-tamanho" title="Excluir Cadastro"><i class="fa fa-close"></i></button></a><?php } else { ?><a href="confexcluiraprovacaoformando.php?id=<?php echo $vetor['id_minhaaprovacao']; ?>&id1=<?php echo $id; ?>&id_formando=<?php echo $id_formando; ?>" ><button type="button" class="btn btn-danger mesmo-tamanho" title="Excluir Cadastro"><i class="fa fa-close"></i></button></a><?php } } ?></td> 
                </tr>
                <?php } ?>
                </tbody>
                <tfoot>
