...
$sql_turma_loop = mysqli_query($con, "SELECT * FROM formandos WHERE turma = '$id_turma' order by nome ASC");
while ($turma = mysqli_fetch_array($sql_turma_loop)) {
	$dompdf = new Dompdf();
	$sql_venda = mysqli_query($con, "select * from vendas where id_formando = '$turma[id_formando]' and tipo = '3' and iniciada='2'");
	if (mysqli_num_rows($sql_venda) > 0) {
		$vetor_venda = mysqli_fetch_array($sql_venda);
		$sql_dup = mysqli_query($con, "select * from duplicatas where id_venda = '$vetor_venda[id_venda]'");
		$vetor_dup = mysqli_fetch_array($sql_dup);
		$sql_duplicatas = mysqli_query($con, "select * from duplicatas_faturas where id_duplicata = '$vetor_dup[id_duplicata]' order by posicao ASC");
		$sql_turma = mysqli_query($con, "select * from turmas where id_turma = '$turma[turma]'");
		...
		$sql_duplicata_consulta = mysqli_query($con, "select * from duplicatas where id_venda = '$vetor_venda[id_venda]'");
		...
		if ($vetor_venda_avulsa['valorentrada'] == 0.00) {
			...
			$valorfinal = $vetor_venda['valorvenda'] - $vetor_venda_avulsa['valorentrada'];
		...
		$valorvendatotal = number_format($vetor_venda['valorvenda'], 2, ',', '.');
		$valorvendaextenso = strtoupper(Monetary::numberToExt($vetor_venda['valorvenda']));
		$valorparcela = $valorfinal / $qtdparcelasfinal;
...
<tr>
<td>
Endereço Completo: '.$turma['endereco'].' '.$turma['complemento'].' '.$turma['numero'].'
</td>
</tr>
...
<tr>
<td>
Cidade: '.$turma['cidade'].'   UF: '.$turma['estado'].'   CEP: '.$turma['cep'].'
</td>
</tr>

<tr>
<td>
Telefone Resid.: '.$turma['telefone'].'    Comercial: '.$turma['telcomercial'].'   Celular: '.$turma['celular'].'
</td>
</tr>
...
<table width="40%" class="table table-bordered table-striped">';
		$sql_eventos = mysqli_query($con, "select * from eventos_venda_avulsa WHERE id_avulsa = '$vetor_venda[produto]' order by id_evento_pacote ASC");
...
<td width="95%">Produtos</td>
		...
		$sql_itens = mysqli_query($con, "select * from venda_avulsa_produtos WHERE id_avulsa = '$vetor_venda[produto]' order by id_produto ASC");
		$i = 1;
...
<table width="100%" BORDER="1" style="border-collapse: collapse">
    ...
    <tr>
		...
		$sql_arquivo = mysqli_query($con, "select * from arquivos where id_cliente = '$turma[id_formando]' and titulo='Contrato Venda Avulsa'");
		if (mysqli_num_rows($sql_arquivo) == 0) {
			...
			$arquivo = mysqli_fetch_array($sql_arquivo);
			$nomegravabanco = $arquivo['arquivo'];
			$unlink = $SERVER_ROOT.'/sistema/arquivos/'.$nomegravabanco;
			unlink($unlink);
		}
		...
		$teste = file_put_contents("../sistema/arquivos/$nomegravabanco", $output);
