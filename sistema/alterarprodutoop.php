<?php
    ...
    $sql = mysqli_query($con, "select * from produtos_formando where id_produto = '$id'");
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title">Produtos do Formando: <?php echo $vetor_formando['nome']; ?></h4>

                                <form action="recebe_alterarprodutoformando.php?id=<?php echo $id; ?>" method="post" name="cliente" enctype="multipart/form-data" onSubmit="return verificarCPF()" id="formID">

                                <ul class="nav nav-tabs" role="tablist">

                                <li class="nav-item"> <a class="nav-link active" data-toggle="tab" href="#album" role="tab"><span class="hidden-sm-up"><i class="ti-email"></i></span> <span class="hidden-xs-down">Álbum</span></a> </li>
                                <li class="nav-item"> <a class="nav-link" data-toggle="tab" href="#produtosadicionais" role="tab"><span class="hidden-sm-up"><i class="ti-email"></i></span> <span class="hidden-xs-down">Produtos Adicionais</span></a> </li>
                                ...
                                <div class="row">
                                  ...
                                  <div class="col-lg-6">
                                    <fieldset class="form-group">
                                      <label class="form-label semibold" for="exampleInput">Qtd de Páginas do Álbum</label>
                                      <input type="number" name="qtd" value="<?php echo $vetor['qtd']; ?>" class="form-control" placeholder="Qtd de Páginas do Álbum" required="">
                                    </fieldset>
                                  </div>

                                  <div class="col-lg-6">
                                    <fieldset class="form-group">
                                      <label class="form-label semibold" for="exampleInput">Encadernação?</label>
                                      <select name="encadernacao" class="form-control" required="">
                                        <option value="1" <?php if (strcasecmp($vetor['encadernacao'], '1') == 0) : ?>selected="selected"<?php endif; ?>>Não</option>
                                        <option value="2" <?php if (strcasecmp($vetor['encadernacao'], '2') == 0) : ?>selected="selected"<?php endif; ?>>Sim</option>
                                      </select>
                                    </fieldset>
                                  </div>
                                ...
                                <div class="tab-pane" id="produtosadicionais" role="tabpanel">
                                  ...
                                  <div class="col-lg-6">
                                    <fieldset class="form-group">
                                      ...
                                      <select name="id_tipo[]" class="form-control">
                                            ...
                                            $sql_produtos = mysqli_query($con, "select * from tipo_opcionais order by nome ASC");

                                            while ($vetor_produto=mysqli_fetch_array($sql_produtos)) { 

                                            ?>
                                            <option value="<?php echo $vetor_produto['id_tipo']; ?>" <?php if (strcasecmp($vetor['id_tipo'], $vetor_produto['id_tipo']) == 0) : ?>selected="selected"<?php endif; ?>><?php echo $vetor_produto['nome'] ?></option>
                                            <?php } ?>
                                          </select>
                                    </fieldset>
                                  </div>
                                      ...
                                      <div class="col-lg-6">
                                        <fieldset class="form-group">
                                          <label class="form-label semibold" for="exampleInput">Valor</label>
                                          <input type="text" name="valor[]" class="form-control" id="exampleInput" placeholder="Valor" onKeyPress="mascara(this,mvalor)" required>
                                        </fieldset>
                                      </div>
                                    ...
                                    <h3>Produtos Opcionais Cadastrados</h3>
                                    ...
                                    <table id="lang_opt" class="table table-bordered table-striped">
                                            ...
                                            $sql_itens = mysqli_query($con, "select * from produtos_opcionais WHERE id_produto = '$id' order by id_item DESC");
                                    
                                            while ($vetor_item=mysqli_fetch_array($sql_itens)) {

                                            $sql_produto = mysqli_query($con, "select * from tipo_opcionais where id_tipo = '$vetor_item[id_tipo]'");
                                            $vetor_produto = mysqli_fetch_array($sql_produto);
                                    
                                            ?>
                                            <tr>
                                              <td><?php echo $vetor_produto['nome']; ?></td>
                                              <td><?php echo $num = number_format($vetor_item['valor'],2,',','.'); ?></td>
                                              <td><a href="alterarprodutoopvenda.php?id=<?php echo $vetor_item['id_item']; ?>&id1=<?php echo $id; ?>" target="_blank"><button type="button" class="btn btn-info mesmo-tamanho" title="Ver ou Alterar Cadastro"><i class="fa fa-edit"></i></button></a> <?php if($vetor_permissao['exclusao'] == 1) { } else { ?><a href="confexcluirprodutoopvenda.php?id=<?php echo $vetor_item['id_item']; ?>&id1=<?php echo $id; ?>" ><button type="button" class="btn btn-danger mesmo-tamanho" title="Excluir Cadastro"><i class="fa fa-close"></i></button></a><?php } ?></td> 
                                            </tr>
                                            <?php } ?>
                                            </tbody>
                                          </table>
