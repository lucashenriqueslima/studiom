...
while ($turma = mysqli_fetch_array($sql_turma_loop)) {
	$dompdf = new Dompdf();
	$sql_venda = mysqli_query($con, "select * from vendas where id_formando = '$turma[id_formando]' and tipo = '2' and formapag <> '18' and iniciada='2'");
...
<table width="40%" class="table table-bordered table-striped">';
		$sql_eventos = mysqli_query($con, "select * from eventos_pacote WHERE id_pacote = '$vetor_venda[id_pacote]' order by id_evento_pacote ASC");
...
<td width="95%">Produtos</td>
		...
		$sql_itens = mysqli_query($con, "select * from pacotes_itens_produtos WHERE id_pacote = '$vetor_venda[id_pacote]' order by id_produto_item ASC");
		$i = 1;
...
<table width="100%" BORDER="1" style="border-collapse: collapse">
    ...
    <tr>
		...
		$sql_arquivo = mysqli_query($con, "select * from arquivos where id_cliente = '$turma[id_formando]' and titulo='Contrato Álbum Formando'");
		if (mysqli_num_rows($sql_arquivo) == 0) {
			$nomeRelatorio = time().uniqid(md5());
			$nomegravabanco = $nomeRelatorio.'.pdf';
			$sql_grava_arquivo = mysqli_query($con, "insert into arquivos (id_cliente, titulo, data, hora, tipo, arquivo) VALUES ('$vetor_venda[id_formando]', 'Contrato Álbum Formando', '$data', '$hora', '1', '$nomegravabanco')");
		}else {
