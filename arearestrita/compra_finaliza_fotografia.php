<?php

class Monetary
{
    const MIN = 0.01;
    const MAX = 2147483647.99;
    const MOEDA = " real ";
    const MOEDAS = " reais ";
    const CENTAVO = " centavo ";
    const CENTAVOS = " centavos ";
    private static $unidades = array(
        "um",
        "dois",
        "três",
        "quatro",
        "cinco",
        "seis",
        "sete",
        "oito",
        "nove",
        "dez",
        "onze",
        "doze",
        "treze",
        "quatorze",
        "quinze",
        "dezesseis",
        "dezessete",
        "dezoito",
        "dezenove"
    );
    private static $dezenas = array(
        "dez",
        "vinte",
        "trinta",
        "quarenta",
        "cinqüenta",
        "sessenta",
        "setenta",
        "oitenta",
        "noventa"
    );
    private static $centenas = array(
        "cem",
        "duzentos",
        "trezentos",
        "quatrocentos",
        "quinhentos",
        "seiscentos",
        "setecentos",
        "oitocentos",
        "novecentos"
    );
    private static $milhares = array(
        array("text" => "mil", "start" => 1000, "end" => 999999, "div" => 1000),
        array("text" => "milhão", "start" => 1000000, "end" => 1999999, "div" => 1000000),
        array("text" => "milhões", "start" => 2000000, "end" => 999999999, "div" => 1000000),
        array("text" => "bilhão", "start" => 1000000000, "end" => 1999999999, "div" => 1000000000),
        array("text" => "bilhões", "start" => 2000000000, "end" => 2147483647, "div" => 1000000000)
    );

    static function numberToExt($number, $moeda = true)
    {
        if ($number >= self::MIN && $number <= self::MAX) {
            $value = self::conversionR((int)$number);
            if ($moeda) {
                if (floor($number) == 1) {
                    $value .= self::MOEDA;
                }else {
                    if (floor($number) > 1) {
                        $value .= self::MOEDAS;
                    }
                }
            }
            $decimals = self::extractDecimals($number);
            if ($decimals > 0.00) {
                $decimals = round($decimals * 100);
                $value .= " e ".self::conversionR($decimals);
                if ($moeda) {
                    if ($decimals == 1) {
                        $value .= self::CENTAVO;
                    }else {
                        if ($decimals > 1) {
                            $value .= self::CENTAVOS;
                        }
                    }
                }
            }
        }
        return trim($value);
    }

    static function conversionR($number)
    {
        if (in_array($number, range(1, 19))) {
            $value = self::$unidades[$number - 1];
        }else {
            if (in_array($number, range(20, 90, 10))) {
                $value = self::$dezenas[floor($number / 10) - 1]." ";
            }else {
                if (in_array($number, range(21, 99))) {
                    $value = self::$dezenas[floor($number / 10) - 1]." e ".self::conversionR($number % 10);
                }else {
                    if (in_array($number, range(100, 900, 100))) {
                        $value = self::$centenas[floor($number / 100) - 1]." ";
                    }else {
                        if (in_array($number, range(101, 199))) {
                            $value = ' cento e '.self::conversionR($number % 100);
                        }else {
                            if (in_array($number, range(201, 999))) {
                                $value = self::$centenas[floor($number / 100) - 1]." e ".self::conversionR($number % 100);
                            }else {
                                foreach (self::$milhares as $item) {
                                    if ($number >= $item['start'] && $number <= $item['end']) {
                                        $value = self::conversionR(floor($number / $item['div']))." ".$item['text']." ".self::conversionR($number % $item['div']);
                                        break;
                                    }
                                }
                            }
                        }
                    }
                }
            }
        }
        return $value;
    }

    private static function extractDecimals($number)
    {
        return $number - floor($number);
    }
}

function Mask($mask, $str)
{
    $str = str_replace(" ", "", $str);
    for ($i = 0; $i < strlen($str); $i++) {
        $mask[strpos($mask, "#")] = $str[$i];
    }
    return $mask;
}

include "../includes/conexao.php";
session_start();
if ($_SESSION['id_formando'] == null || $_SESSION['id_formando'] == '') {
    echo "<script language=\"JavaScript\">
    location.href=\"index.php\";
    </script>";
}else {
    $id_item = $_POST['id_item'];
    $formapag = $_POST['formapag'];
    $diavencimento = $_POST['diavencimento'];
    if (isset($_POST['qtdparcelas']) and $_POST['qtdparcelas'] > 0) {
        $qtdparcelas = $_POST['qtdparcelas'];
    }else {
        $qtdparcelas = 1;
    }
    $id_venda = $_GET['id'];
    $formando = mysqli_fetch_array(mysqli_query($con, "select * from formandos where id_formando = '{$_SESSION['id_formando']}'"));
    $dataaceite = date('d/m/Y');
   
    $horaatual = date('H:i:s');
    $ip = $_SERVER['REMOTE_ADDR'];

    $vetor_cadastro = mysqli_fetch_array(mysqli_query($con, "select * from formandos where id_formando = '$_SESSION[id_formando]'"));
    $vetor_turma = mysqli_fetch_array(mysqli_query($con, "select * from turmas where id_turma = '$vetor_cadastro[turma]'"));
    $vetor_curso = mysqli_fetch_array(mysqli_query($con, "select * from cursos where id_curso = '$vetor_turma[curso]'"));
    $vetor_instituicao = mysqli_fetch_array(mysqli_query($con, "select * from instituicoes where id_instituicao = '$vetor_curso[id_instituicao]'"));
    $dataatual = date('Y-m-d');
    $vetor_forma = mysqli_fetch_array(mysqli_query($con, "select * from formaspag where id_forma = '$formapag'"));
    $pacote_itens = mysqli_fetch_array(mysqli_query($con, "select * from pacotes_itens_album where id_item = '$id_item'"));
    $pacote = mysqli_fetch_array(mysqli_query($con, "select * from pacotes where id_pacote = '$pacote_itens[id_pacote]'"));
    $valorfinal = 0;
    if ($vetor_cadastro['comissao'] == 2) {
        $valorfinal = $pacote_itens['valorcomissao'];
    }else {
        $valorfinal = $pacote_itens['valor'];
    }
    if ($formapag == '18') {
        $datadesconto = $pacote['dataabertura'];
        $dataatual = date('Y-m-d');
        $calcula_dias = strtotime($dataatual) - strtotime($datadesconto);
        $diferenca_dias = (int)floor($calcula_dias / (60 * 60 * 24));
        if ($diferenca_dias <= 30 && ($pacote_itens['pacote_especial'] == 2 || $pacote_itens['pacote_especial'] == 1)) {
            $valorfinal = $valorfinal - ($valorfinal * $pacote['desconto'] / 100);
        }
    }


function extenso($value, $uppercase = 0)
{
    if (strpos($value, ",") > 0) {
        $value = str_replace(".", "", $value);
        $value = str_replace(",", ".", $value);
    }
    $singular = ["centavo", "real", "mil", "milhão", "bilhão", "trilhão", "quatrilhão"];
    $plural = ["centavos", "reais", "mil", "milhões", "bilhões", "trilhões", "quatrilhões"];
 
    $c = ["", "cem", "duzentos", "trezentos", "quatrocentos", "quinhentos", "seiscentos", "setecentos", "oitocentos", "novecentos"];
    $d = ["", "dez", "vinte", "trinta", "quarenta", "cinquenta", "sessenta", "setenta", "oitenta", "noventa"];
    $d10 = ["dez", "onze", "doze", "treze", "quatorze", "quinze", "dezesseis", "dezesete", "dezoito", "dezenove"];
    $u = ["", "um", "dois", "três", "quatro", "cinco", "seis", "sete", "oito", "nove"];
 
    $z = 0;
 
    $value = number_format($value, 2, ".", ".");
    $integer = explode(".", $value);
    $cont = count($integer);
    for ($i = 0; $i < $cont; $i++)
        for ($ii = strlen($integer[$i]); $ii < 3; $ii++)
            $integer[$i] = "0" . $integer[$i];
 
    $fim = $cont - ($integer[$cont - 1] > 0 ? 1 : 2);
    $rt = '';
    for ($i = 0; $i < $cont; $i++) {
        $value = $integer[$i];
        $rc = (($value > 100) && ($value < 200)) ? "cento" : $c[$value[0]];
        $rd = ($value[1] < 2) ? "" : $d[$value[1]];
        $ru = ($value > 0) ? (($value[1] == 1) ? $d10[$value[2]] : $u[$value[2]]) : "";
 
        $r = $rc . (($rc && ($rd || $ru)) ? " e " : "") . $rd . (($rd &&
                $ru) ? " e " : "") . $ru;
        $t = $cont - 1 - $i;
        $r .= $r ? " " . ($value > 1 ? $plural[$t] : $singular[$t]) : "";
        if ($value == "000"
        )
            $z++;
        elseif ($z > 0)
            $z--;
        if (($t == 1) && ($z > 0) && ($integer[0] > 0))
            $r .= ( ($z > 1) ? " de " : "") . $plural[$t];
        if ($r)
            $rt = $rt . ((($i > 0) && ($i <= $fim) &&
                    ($integer[0] > 0) && ($z < 1)) ? ( ($i < $fim) ? ", " : " e ") : " ") . $r;
    }
 
    if (!$uppercase) {
        return trim($rt ? $rt : "zero");
    } elseif ($uppercase == "2") {
        return trim(strtoupper($rt) ? strtoupper(strtoupper($rt)) : "Zero");
    } else {
        return trim(ucwords($rt) ? ucwords($rt) : "Zero");
    }
}

    ?>
    <!DOCTYPE html>
    <html dir="ltr" lang="en">

    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <!-- Tell the browser to be responsive to screen width -->
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <meta name="description" content="">
        <meta name="author" content="">
        <!-- Favicon icon -->
        <link rel="icon" type="image/png" sizes="16x16" href="../layout/assets/images/favicon.png">
        <title>Studio M Fotografia</title>
        <!-- Custom CSS -->
        <link href="../layout/assets/libs/chartist/dist/chartist.min.css" rel="stylesheet">
        <link href="../layout/assets/extra-libs/c3/c3.min.css" rel="stylesheet">

        <link href="../layout/assets/extra-libs/datatables.net-bs4/css/dataTables.bootstrap4.css" rel="stylesheet">
        <!-- Custom CSS -->
        <link href="../layout/dist/css/style.min.css" rel="stylesheet">

        <script type="text/javascript" src="../aplicacoes/aplicjava.js"></script>

        <script src="//code.jquery.com/jquery-2.1.4.min.js"></script>
        <script src="../layout/assets/libs/tinymce/tinymce.min.js"></script>

        <script>
            $(function () {
                if ($("#mymce").length > 0) {
                    tinymce.init({
                        selector: "textarea#mymce",
                        menubar: false,
                        toolbar: false,
                        branding: false,
                        readonly: true,
                        height: 500,
                        alignjustify: { selector: 'p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li,table,img', classes: 'full' }
                    });
                }
                if ($("#mymce2").length > 0) {
                    tinymce.init({
                        selector: "textarea#mymce2",
                        menubar: false,
                        toolbar: false,
                        branding: false,
                        readonly: true,
                        height: 500,
                        alignjustify: { selector: 'p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li,table,img', classes: 'full' }
                    });
                }
            });
        </script>
        <style type="text/css">
            #license-box {
                font-family: Verdana, Arial, Sans-Serif;
                width: 100%;
                padding: 20px 30px;
                border-radius: 5px;
                box-shadow: 1px 1px 10px #999;
                margin: auto;

                background: #ffffff; /* Old browsers */
                background: -moz-linear-gradient(top, #ffffff 0%, #f3f3f3 50%, #ededed 51%, #ffffff 100%); /* FF3.6+ */
                background: -webkit-gradient(linear, left top, left bottom, color-stop(0%, #ffffff), color-stop(50%, #f3f3f3), color-stop(51%, #ededed), color-stop(100%, #ffffff)); /* Chrome,Safari4+ */
                background: -webkit-linear-gradient(top, #ffffff 0%, #f3f3f3 50%, #ededed 51%, #ffffff 100%); /* Chrome10+,Safari5.1+ */
                background: -o-linear-gradient(top, #ffffff 0%, #f3f3f3 50%, #ededed 51%, #ffffff 100%); /* Opera 11.10+ */
                background: -ms-linear-gradient(top, #ffffff 0%, #f3f3f3 50%, #ededed 51%, #ffffff 100%); /* IE10+ */
                background: linear-gradient(to bottom, #ffffff 0%, #f3f3f3 50%, #ededed 51%, #ffffff 100%); /* W3C */
                filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='#ffffff', endColorstr='#ffffff', GradientType=0); /* IE6-9 */
            }

            #license-box h1 {
                font-size: 20px;
                color: #333333;
                text-transform: uppercase;
                text-align: center;
            }

            #license-box .text textarea {
                width: 100%;
                height: 400px;
                border-color: #f1f1f1;
                background-color: #fff;
            }

            #license-box .next {
                padding: 15px 0 0 0;
                float: right;
            }

            #license-box .next input {
                padding: 5px 20px;
                color: #fff;
                font-weight: bold;
                border-width: 0;
                border-radius: 5px;
                box-shadow: 1px 1px 5px #666;
                cursor: pointer;
                background: #b4e391; /* Old browsers */
                background: -moz-linear-gradient(top, #b4e391 0%, #61c419 50%, #b4e391 100%); /* FF3.6+ */
                background: -webkit-gradient(linear, left top, left bottom, color-stop(0%, #b4e391), color-stop(50%, #61c419), color-stop(100%, #b4e391)); /* Chrome,Safari4+ */
                background: -webkit-linear-gradient(top, #b4e391 0%, #61c419 50%, #b4e391 100%); /* Chrome10+,Safari5.1+ */
                background: -o-linear-gradient(top, #b4e391 0%, #61c419 50%, #b4e391 100%); /* Opera 11.10+ */
                background: -ms-linear-gradient(top, #b4e391 0%, #61c419 50%, #b4e391 100%); /* IE10+ */
                background: linear-gradient(to bottom, #b4e391 0%, #61c419 50%, #b4e391 100%); /* W3C */
                filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='#b4e391', endColorstr='#b4e391', GradientType=0); /* IE6-9 */
            }

            #license-box .next input:disabled {
                background: #e2e2e2; /* Old browsers */
                background: -moz-linear-gradient(top, #e2e2e2 0%, #dbdbdb 50%, #d1d1d1 51%, #fefefe 100%); /* FF3.6+ */
                background: -webkit-gradient(linear, left top, left bottom, color-stop(0%, #e2e2e2), color-stop(50%, #dbdbdb), color-stop(51%, #d1d1d1), color-stop(100%, #fefefe)); /* Chrome,Safari4+ */
                background: -webkit-linear-gradient(top, #e2e2e2 0%, #dbdbdb 50%, #d1d1d1 51%, #fefefe 100%); /* Chrome10+,Safari5.1+ */
                background: -o-linear-gradient(top, #e2e2e2 0%, #dbdbdb 50%, #d1d1d1 51%, #fefefe 100%); /* Opera 11.10+ */
                background: -ms-linear-gradient(top, #e2e2e2 0%, #dbdbdb 50%, #d1d1d1 51%, #fefefe 100%); /* IE10+ */
                background: linear-gradient(to bottom, #e2e2e2 0%, #dbdbdb 50%, #d1d1d1 51%, #fefefe 100%); /* W3C */
                filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='#e2e2e2', endColorstr='#fefefe', GradientType=0); /* IE6-9 */
            }

            #license-box .checkbox {
                padding: 20px 0;
            }

            #license-box .checkbox input {
                margin-right: 5px;
            }

            #license-box .checkbox label {
                font-size: 12px;
                font-weight: bold;
            }

            .circle img {
                background-color: #ddd;
                border-radius: 100%;
                height: 100px;
                width: 100px;
                object-fit: cover;
            }

            p{
                text-align: justify !important;
                display:inline-block !important;
            }
        </style>
        
        
    </head>

    <body>
    <!-- ============================================================== -->
    <!-- Preloader - style you can find in spinners.css -->
    <!-- ============================================================== -->
    <div class="preloader">
        <div class="lds-ripple">
            <div class="lds-pos"></div>
            <div class="lds-pos"></div>
        </div>
    </div>
    <!-- ============================================================== -->
    <!-- Main wrapper - style you can find in pages.scss -->
    <!-- ============================================================== -->
    <div id="main-wrapper">
        <!-- ============================================================== -->
        <!-- Topbar header - style you can find in pages.scss -->
        <!-- ============================================================== -->
        <header class="topbar">
            <nav class="navbar top-navbar navbar-expand-md navbar-dark">
                <div class="navbar-header">
                    <!-- This is for the sidebar toggle which is visible on mobile only -->
                    <a class="nav-toggler waves-effect waves-light d-block d-md-none" href="javascript:void(0)"><i
                                class="ti-menu ti-close"></i></a>
                    <!-- ============================================================== -->
                    <!-- Logo -->
                    <!-- ============================================================== -->
                    <a class="navbar-brand" href="inicio.php">
                        <b class="logo-icon">

                            <img src="../layout/assets/images/logo-2.png" alt="homepage" class="dark-logo"
                                 width="110px"/>

                            <img src="../layout/assets/images/logo-icon.png" alt="homepage" class="light-logo"
                                 width="50px"/>
                        </b>

                    </a>

                    <a class="topbartoggler d-block d-md-none waves-effect waves-light" href="javascript:void(0)"
                       data-toggle="collapse" data-target="#navbarSupportedContent"
                       aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation"><i
                                class="ti-more"></i></a>
                </div>

                <div class="navbar-collapse collapse" id="navbarSupportedContent">

                    <ul class="navbar-nav float-left mr-auto">
                        <li class="nav-item d-none d-md-block"><a
                                    class="nav-link sidebartoggler waves-effect waves-light" href="javascript:void(0)"
                                    data-sidebartype="mini-sidebar"><i class="mdi mdi-menu font-24"></i></a></li>


                    </ul>

                    <ul class="navbar-nav float-right">


                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle text-muted waves-effect waves-dark pro-pic" href=""
                               data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><img
                                        src="../sistema/arquivos/<?php echo $vetor_cadastro['imagem']; ?>" alt="user"
                                        class="rounded-circle" width="31"></a>
                            <div class="dropdown-menu dropdown-menu-right user-dd animated flipInY">
                                <span class="with-arrow"><span class="bg-primary"></span></span>
                                <div class="d-flex no-block align-items-center p-15 bg-primary text-white m-b-10">
                                    <div class=""><img
                                                src="../sistema/arquivos/<?php echo $vetor_cadastro['imagem']; ?>"
                                                alt="user" class="img-circle" width="60"></div>
                                    <div class="m-l-10">
                                        <h4 class="m-b-0"><?php echo $vetor_cadastro['nome']; ?></h4>
                                    </div>
                                </div>
                                <div class="dropdown-divider"></div>
                                <a class="dropdown-item" href="sair.php"><i class="fa fa-power-off m-r-5 m-l-5"></i>
                                    Sair</a>
                            </div>
                        </li>
                        <!-- ============================================================== -->
                        <!-- User profile and search -->
                        <!-- ============================================================== -->
                    </ul>
                </div>
            </nav>
        </header>
        <!-- ============================================================== -->
        <!-- End Topbar header -->
        <!-- ============================================================== -->
        <!-- ============================================================== -->
        <!-- Left Sidebar - style you can find in sidebar.scss  -->
        <!-- ============================================================== -->
        <?php include "includes/menu.php"; ?>
        <!-- ============================================================== -->
        <!-- End Left Sidebar - style you can find in sidebar.scss  -->
        <!-- ============================================================== -->
        <!-- ============================================================== -->
        <!-- Page wrapper  -->
        <!-- ============================================================== -->
        <div class="page-wrapper">
            <!-- ============================================================== -->
            <!-- Bread crumb and right sidebar toggle -->
            <!-- ============================================================== -->
            <div class="page-breadcrumb">
                <div class="row">
                    <div class="col-5 align-self-center">
                        <h4 class="page-title">Minhas Compras</h4>
                        <div class="d-flex align-items-center">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item">Home</a></li>
                                    <li class="breadcrumb-item active" aria-current="page">Minhas Compras</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                </div>
            </div>
            <!-- ============================================================== -->
            <!-- End Bread crumb and right sidebar toggle -->
            <!-- ============================================================== -->
            <!-- ============================================================== -->
            <!-- Container fluid  -->
            <!-- ============================================================== -->
            
            <div class="container-fluid">
                <!-- ============================================================== -->
                <!-- Sales chart -->
                <!-- ============================================================== -->
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                            
                                <h4 class="card-title">Minhas Compras</h4>
                                <?php if (($pacote['tipoContrato'] != 2) && ($pacote['tipoContrato'] != 3 ) && ($pacote['tipoContrato'] != 4 )) {
                                    # Contrato Antigo
                                ?>

                                <form method="post" action="compra_assinatura_fotografia.php?id=<?= $id_item?>">
                                    <div id="license-box">
                                        <?php if ($pacote_itens['pacote_especial'] == '3') { ?>
                                            <div class="text">
                                    <textarea id="mymce" disabled="true">
                                        <table width="100%">
                                            <tr>
                                                <td width="50%"><img src="../imgs/logo1.png" width="100px"></td>
                                                <td width="50%" align="center">(62)3218-3476
                                                    <br>
                                                    Rua 93 nº 296 qd. F-14 lt. 36
                                                    <br>
                                                    St. Sul, Goiânia-GO - CEP 74083-120
                                                    <br>
                                                    cliente@studiomfotografia.com.br
                                                </td>
                                            </tr>
                                        </table>
                                        <div>
                                        <p>Parabéns por sua aquisição <?php echo $vetor_cadastro['nome']; ?></p>
                                        <p>Você faz parte do contrato: <?php echo $vetor_turma['ncontrato'] . ' - ' . $vetor_curso['nome'] . ' ' . $vetor_instituicao['nome'] . ' - '. $vetor_instituicao['cidade'] . ' - ' . $vetor_instituicao['estado']; ?></p>

                                        <p>Este é o extrato de sua compra:</p>

                                        <table width="50%">
                                            <tr>
                                                <th><strong><h3>Produto</h3></strong></th>
                                                <th><strong><h3>Quantidade</h3></strong></th>
                                                <th><strong><h3>Sub-Total</h3></strong></th>
                                            </tr>
                                            <tr>
                                                <td align="center">Fotos para Convite</td>
                                                <td align="center">01 un.</td>
                                                <td align="center">R$<?php echo number_format($pacote_itens['valor'], 2, ',', '.'); ?></td>
                                            </tr>
                                            <tr>
                                                <td align="center"></td>
                                                <td align="center"></td>
                                                <td align="center">Total: R$<?php echo number_format($pacote_itens['valor'], 2, ',', '.'); ?></td>
                                            </tr>
                                        </table>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Sua forma de pagamento escolhida é: <?php echo $vetor_forma['nome']; ?> em <?php echo $qtdparcelas; ?> parcela(s).</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">O produto “Fotos para Convite” consiste especificamente nas fotografias a serem produzidas para o convite de formatura, exceto as fotos de Família. Tais exemplos de fotografias estão especificados no documento de “Orientação Fotográfica”, advindo do convite de formatura.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Após a realização das fotos para o convite, as mesmas serão disponibilizadas para a escolha através da plataforma digital da empresa para que possam receber o (s) devido (s) tratamento (s) indicados por você. O prazo para que isto aconteça é de até 45 dias úteis e/ou prazo estipulado entre a comissão de formatura e a Studio M Fotografia.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">A entrega destas fotos tratadas também será através da plataforma digital logo após o tratamento das imagens.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Ficará a cargo da comissão de formatura a solicitação do agendamento dos dias para a realização do evento “Ensaio Foto de Convite”, o qual faz parte as “Foto para Convite”.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">O associado que, por qualquer motivo, cancelar a compra ou não participar das “Fotos para o Convite” não terá direito a ressarcimento, pois o mesmo fará ou fez parte do planejamento de gastos das “Fotos para Convite”.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Declaro para todos os fins de direito que, efetuei a compra das “Fotos para Convite” e estou ciente que o valor cobrado por tal produto está presente no contrato de fotografia, o qual rege todas as regras da prestação do serviço.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Concordo e aprovo todos os compromissos que a comissão de formatura venha assumir em meu nome para a realização para as “Fotos para Convite”, acatando as decisões e medidas por ela tomadas, e, por ser verdade, dato e confirmo a a compra das “Fotos para Convite”.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Assim sendo, na condição de contratante, ciente das minhas obrigações contratuais, firmo meu ACEITE na compra do produto “Fotos para Convite” o qual é parte integrante do contrato de fotografia.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Aceite da compra feito por meio de confirmação via sistema recebido pelo link de confirmação:</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">
                                            E-mail: <?php echo $vetor_cadastro['email']; ?> <br>
                                            Data Aceite: <br>
                                            Hora Aceite:  <br>
                                            IP de Acesso:  <br>
                                        </p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">
                                            Atenciosamente; <br>
                                            StudioM Fotografia
                                        </p>
                                    </textarea>
                                            </div>
                                        <?php }else { ?>
                                            <h1>Contrato Individual</h1>
                                            <div class="text">
                                    <textarea id="mymce2" disabled="true">
                                        <table width="100%">
                                            <tr>

                                                <td width="50%"><img src="data:image/png;base64," alt="" width="250px" /></td>
                                                <td width="50%">(62)3218-3476
                                                    <br>
                                        Rua 93 nº 296 qd. F-14 lt. 36
                                        <br>
                                        St. Sul, Goiânia-GO - CEP 74083-120
                                        <br>
                                        cliente@studiomfotografia.com.br
                                        </td>
                                            </tr>
                                        </table>
                                        <br>
                                        <div align="right">Contrato n° <?php echo $vetor_turma['ncontrato']; ?>></div>
                                        <p>
                                        </p>
                                        <p>
                                        </p>

                                        <p><strong><div align="center">CONTRATO INDIVIDUAL DE PRESTAÇÃO DE SERVIÇOS FOTOGRÁFICOS</div></strong></p>

                                        <p>
                                        </p>
                                        <table>

                                        <tr>
                                        <td>
                                        <p><strong>1 - PARTES CONTRATANTES</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="text-align: justify;" data-mce-style="text-align: justify;"><strong>a) VENDEDORA CONTRATADA</strong>, doravante denominada simplesmente por STUDIO M FOTOGRAFIA, razão social: Studio M Fotografia EIRELI, CNPJ: 16.683.613/0001-00 e endereço da sede à rua 93 nº 296 qd. F-14 lt. 36, Setor Sul, Goiânia-GO.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>b) COMPRADOR CONTRATANTE</strong>, doravante designado apenas CONTRATANTE:</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>Nome:</strong> <?php echo $vetor_cadastro['nome']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        RG: <?php echo $vetor_cadastro['rg']; ?>       CPF: <?php echo $vetor_cadastro['cpf']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        Endereço Completo: <?php echo $vetor_cadastro['endereco'] . '  ' .$vetor_cadastro['complemento']. '  ' . $vetor_cadastro['numero']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        Setor: <?php echo $vetor_cadastro['bairro']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        Cidade: <?php echo $vetor_cadastro['cidade']; ?>   UF: <?php echo $vetor_cadastro['estado']; ?>   CEP: <?php echo $vetor_cadastro['cep']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        Telefone Resid.: <?php echo $vetor_cadastro['telefone']; ?>   Comercial: <?php echo $vetor_cadastro['telcomercial']; ?>   Celular: <?php echo $vetor_cadastro['celular']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        E-mail: <?php echo $vetor_cadastro['email']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>2 - DO OBJETO DO CONTRATO</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>2.1 O presente instrumento de Contrato de Prestação de Serviços tem por objeto a Comercialização de álbum de fotografia, nas condições abaixo estabelecidas.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>2.1.1 Compreende-se o produto <b>álbum de fotografia</b> o material fotográfico impresso e mídia digital disponibilizado pela CONTRATADA ao CONTRATANTE.</p>
                                        </td>
                                        </tr>
                                        <tr>
                                        <td>
                                        <p>2.2 integra ao objeto do presente contrato a prestação de serviços fotográficos para a realização de fotos para o convite de formatura, cujo serviço terá o preço adicional de <b style="text-decoration: underline">R$ 250,00 (Duzentos e cinquenta reais)</b>, incluso nesse valor, as fotografias indicadas em orientação fotográfica, provenientes do projeto do convite de formatura, <b>exceto</b> as fotos de família.</p>
                                        </td>
                                        </tr>

                                        
                                            
                                        <tr>
                                        <td>
                                        <p>2.2.1 A compra desse serviço deverá ser realizada através da plataforma digital (<span style="color: blue;text-decoration: underline;">www.studiomfotografia.com.br</span> - área do formando),  em até 30 dias antes da data de realização das fotos para o convite.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>3 – DOS EVENTOS CONTRATADOS</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <table width="40%" class="table table-bordered table-striped">
                                                <?php
                                                $sql_eventos = mysqli_query($con, "select * from eventos_pacote WHERE id_pacote = '$pacote_itens[id_item]' order by id_evento_pacote ASC");
                                                $i = 0;
                                                while ($vetor_eventos = mysqli_fetch_array($sql_eventos)) {
                                                    $sql_evento = mysqli_query($con, "select * from eventos_turma_lista where id_evento_turma = '$vetor_eventos[id_evento]'");
                                                    $vetor_evento = mysqli_fetch_array($sql_evento);
                                                    $sql_evento_nome = mysqli_query($con, "select * from categoriaevento where id_categoria = '$vetor_evento[id_evento]'");
                                                    $vetor_evento_nome = mysqli_fetch_array($sql_evento_nome);
                                        ?>
                                                    <tr>
                                                    <td><?php if ($vetor_evento_nome['nome'] == 'Pré-evento'){$i++; echo $vetor_evento_nome['nome'].' '.$i; }else { echo $vetor_evento_nome['nome']; }?></td>
                                                    </tr>
                                                <?php
                                                }
                                                ?>


                                        </table>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>4 - DOS PRODUTOS ADQUIRIDOS E PREÇO</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <table width="100%" class="table table-bordered table-striped">
                                        <tr style="    background: #000000; color: #fff;">
                                        <td width="5%">Item</td>
                                        <td width="95%">Produtos</td>
                                        </tr>

                                                                                <?php
                                                                            $sql_itens = mysqli_query($con, "select * from pacotes_itens_produtos WHERE id_pacote = '{$pacote_itens['id_item']}' order by id_produto_item ASC");
                                        $i = 1;

                                        while ($vetor_item=mysqli_fetch_array($sql_itens)) {

                                        $sql_produto = mysqli_query($con, "select * from tipo_opcionais where id_tipo = '{$vetor_item['id_produto']}'");
                                        $vetor_produto = mysqli_fetch_array($sql_produto);

                                        $valorun = number_format($vetor_item['valor'],2,',','.');
                                        ?>
                                        <tr>
                                        <td><?php echo $i; ?></td>
                                        <td><?php echo $vetor_produto['nome']; ?>

                                                                                <?php
                                        if($vetor_produto['id_tipo'] == 10 || $vetor_produto['id_tipo'] == 11 || $vetor_produto['id_tipo'] == 12 || $vetor_produto['id_tipo'] == 13 || $vetor_produto['id_tipo'] == 15) {

                                        echo $vetor_item['qtdpaginas'].' páginas</td>';

                                        }
                                        $i++;
                                        }
                                        ?>
                                        </table>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        4.1 Em razão dos produtos adquiridos, especificamente, pelo CONTRATANTE, as partes têm por certo e ajustado que o valor total dos produtos contratados é de R$ <?php echo $num = number_format($valorfinal, 2, ',', '.'); ?>(<?php echo strtoupper(Monetary::numberToExt($valorfinal)); ?>), considerando a quantidade de páginas do álbum de fotografia e produto (s) acima referenciado (s).
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>5 - FORMAS DE PAGAMENTO</strong>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>O Pagamento será realizado em <?php $valorparcela = $valorfinal / $qtdparcelas;
                                                                                echo $qtdparcelas; ?> parcela (s) de R$<?php echo $num = number_format($valorparcela, 2, ',', '.'); ?> (<?php echo strtoupper(Monetary::numberToExt($valorparcela)); ?>),  através da seguinte opção:</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        • <?php echo $vetor_forma['nome']; ?>.
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>

                                        <table width="100%" class="table table-bordered table-striped">

                                        <tr style="    background: #000000; color: #fff;">
                                        <td width="20%">Parcela</td>
                                        <td width="20%">Valor</td>
                                        <td width="60%">Data de Vencimento</td>
                                        </tr>
                                                                                <?php
                                                                                $i = 1;
                                                                                while ($i <= $qtdparcelas) {
                                                                                if ($formapag != '8') {
                                                                                    if ($formapag == '18') {
                                                                                        $data = date('Y-m-d', strtotime('+4 days'));
                                                                                    }else {
                                                                                        $data = date('Y-m', strtotime('+'.$i.' months'));
                                                                                    }
                                                                                }else {
                                                                                    $data = date('Y-m', strtotime('+'.$i.' months', strtotime(substr($pacote['dataentrega'], 0, 7))));
                                                                                }
                                                                                $dataexplode = explode('-', $data);
                                                                                $mesdup = $dataexplode[1];
                                                                                if ($mesdup == 1) {
                                                                                    $mesgerado1 = "Janeiro";
                                                                                }
                                                                                if ($mesdup == 2) {
                                                                                    $mesgerado1 = "Fevereiro";
                                                                                }
                                                                                if ($mesdup == 3) {
                                                                                    $mesgerado1 = "Março";
                                                                                }
                                                                                if ($mesdup == 4) {
                                                                                    $mesgerado1 = "Abril";
                                                                                }
                                                                                if ($mesdup == 5) {
                                                                                    $mesgerado1 = "Maio";
                                                                                }
                                                                                if ($mesdup == 6) {
                                                                                    $mesgerado1 = "Junho";
                                                                                }
                                                                                if ($mesdup == 7) {
                                                                                    $mesgerado1 = "Julho";
                                                                                }
                                                                                if ($mesdup == 8) {
                                                                                    $mesgerado1 = "Agosto";
                                                                                }
                                                                                if ($mesdup == 9) {
                                                                                    $mesgerado1 = "Setembro";
                                                                                }
                                                                                if ($mesdup == 10) {
                                                                                    $mesgerado1 = "Outubro";
                                                                                }
                                                                                if ($mesdup == 11) {
                                                                                    $mesgerado1 = "Novembro";
                                                                                }
                                                                                if ($mesdup == 12) {
                                                                                    $mesgerado1 = "Dezembro";
                                                                                }
                                                                                $dataduplicata = $mesgerado1.'/'.$dataexplode[0];
                                                                                ?>
                                        <tr>
                                        <td><?php echo $i.'/'.$qtdparcelas; ?></td>
                                        <td>R$ <?php echo $num = number_format($valorparcela, 2, ',', '.'); ?></td>
                                        <td><?php echo $dataduplicata; ?></td>
                                        </tr>
                                        <?php
                                            $i++;
                                        }
                                        ?>
                                        </table>

                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>6 - DO INADIMPLEMENTO DAS PARCELAS E OBRIGAÇÕES</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>6.1 O não pagamento de qualquer parcela acima ajustado na data prevista caracterizará a mora da <strong>CONTRATANTE</strong> e, implicará no vencimento antecipado da dívida, independente de qualquer notificação do <strong>CONTRATANTE</strong>, com a incidência de multa de 2% (dois por cento), sobre a dívida original, acrescido de juros à taxa de 1% (um por cento) ao mês, bem como atualização monetária pelo INPC - Índice Nacional Preço ao Consumidor, além das despesas judiciais e dos honorários advocatórios aqui estabelecidos entre as partes em 20% (vinte por cento), a serem pagos pela <strong>CONTRATANTE</strong> </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>6.2 A rescisão voluntária do contrato ou à que der causa à rescisão motivada da outra, aplicar-se-á multa rescisória de 40% (quarenta por cento) sobre o valor do preço total dos produtos adquiridos, sem prejuízo de aplicação de outras penalidades previstas em lei (art. 389 do Código Civil);</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>6.3 A multa não será aplicada a contratante que por ventura seja impedido acompanhar a respectiva turma de formatura, em razão de transferência de instituição ou em caso de reprovação. Entretanto, ficará sujeito ao pagamento de todos os serviços prestados pela CONTRATADA e já entregue ao CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>7 - DA ENTREGA DO PRODUTO</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1 É obrigação da CONTRATADA entregar os produtos adquiridos pelo CONTRATANTE dentro dos prazos estipulados nas seguintes condições: </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.1 As fotos serão disponibilizadas para a escolha e aprovação do (s) produto (s) <b>exclusivamente</b> pela plataforma digital (<span style="color: blue;text-decoration: underline">www.studiomfotografia.com.br</span> - área do formando); </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.1.1 Em até 45 dias úteis após o último evento da turma, as fotos dos eventos serão disponibilizadas para a escolha através da plataforma digital <b>(7.1.1)</b>. Em conjunto com as fotografias, será(ão) disponibilizado(s) o(s) mostruário(s) piloto do(s) produto(s) no intuito de auxiliar a todos na escolha das fotos.
                                        </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.1.2 A partir da escolha das fotos pelo CONTRATANTE, a CONTRATADA terá o prazo de até 30 dias úteis, para disponibilizar o(s) produto(s) diagramado(s) para a APROVAÇÃO e, se for o caso, a RETIFICAÇÃO solicitada pelo CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.1.3 O(s) produto(s) contratado(s) somente serão produzidos mediante a aprovação do CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.2 A partir da aprovação do(s) produto(s) pelo CONTRATANTE, a CONTRATADA terá o prazo de até 45 dias úteis, para o envio do(s) produto(s) para o endereço indicado pelo CONTRATANTE neste instrumento.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.2.1 A CONTRATADA não será, em nenhuma hipótese, responsável pelo extravio do(s) produto(s) contratado(s) por erro/incorreção no endereço de destino da mercadoria.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.3 Caso o produto fique pendente de escolha <b>(7.1.1.1)</b> e aprovação <b>(7.1.1.2)</b> pelo CONTRATANTE, a CONTRATADA não será responsabilizada por qualquer atraso na entrega do (s) produto (s) contratado (s).</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>8 - DAS CONDIÇÕES GERAIS</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.1 As datas dos eventos extraoficiais e oficiais, incluindo a data para a realização das fotos para o convite de formatura serão informadas previamente pela comissão de formatura, não tendo o CONTRATANTE e CONTRATADA qualquer responsabilidade sobre o agendamento dos mencionados eventos.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.2 <b>As fotos para o convite não serão realizadas em outra data que não seja a data oficial informada pela comissão de formatura, exceto em casos excepcionais</b>, tais como luto, acidente ou outra situação agravante que comprometa o comparecimento ao evento, desde que possa ser comprovado. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.2.1 Caso ocorra a hipótese prevista no item anterior, o CONTRATANTE deverá reagendar a realização das fotos em data de evento disponível pela CONTRATADA, desde que não comprometa o prazo de entrega das fotos para a empresa responsável pela confecção do convite de formatura.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.2.2 Eventual indisponibilidade de data e evento para que sejam realizadas tais fotografias, será de inteira responsabilidade do formando fazê-las por conta própria e enviá-las à empresa responsável pela confecção do convite de formatura, sendo certo que a CONTRATANTE não se responsabilizará por qualquer serviço realizado por terceiros.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.2.3 Caso haja a necessidade de deslocamento entre cidades para a realização das fotografias para o convite, esta será de inteira responsabilidade do CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.3 <b>A CONTRATANTE autoriza expressamente e sem qualquer ônus, a utilização das imagens</b> dos eventos de formatura, para efeitos de divulgação da CONTRATADA, por parte da CONTRATADA</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.4 <b>Fica autorizada a CONTRATADA</b> posicionar sua logomarca, a título de identificação de sua autoria do (s) produto (s) e/ou serviço (s) que com ela forem contratados.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.5 Na hipótese de o CONTRATANTE ficar inadimplente com as obrigações financeiras, os produtos adquiridos por este contrato ficarão retidos até a resolução de tais pendências.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.6 AS PARTES, devidamente nomeadas e qualificadas no preâmbulo deste instrumento, responsabilizam-se solidariamente, em caráter irrevogável e irretratável, pelo cumprimento deste contrato.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.7 A obrigação ora reconhecida e assumida pelos CONTRATANTES, como líquida, certa e exigível, no valor  mencionado no <b>item 4.1</b>, aplica-se o disposto no artigo 784 ,III do Código de Processo Civil, vez que possui força de <b>Título Executivo Extrajudicial</b>.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>9 – DA PROTEÇÃO DE DADOS PESSOAIS</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1 A CONTRATADA, por si e por seus colaboradores, obriga- se a atuar no presente Contrato em conformidade com a Legislação vigente sobre Proteção de Dados Pessoais e as determinações de órgãos reguladores/fiscalizadores sobre a matéria, em especial a Lei 13.709/2018, além das demais normas e políticas de proteção de dados de cada país onde houver qualquer tipo de tratamento dos dados dos clientes, o que inclui os dados dos clientes desta. No manuseio dos dados a CONTRATADA deverá:</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.1 Tratar os dados pessoais a que tiver acesso apenas de acordo com as instruções da CONTRATANTE e em conformidade com estas cláusulas, e que, na eventualidade, de não mais poder cumprir estas obrigações, por qualquer razão, concorda em informar de modo formal este fato imediatamente à CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.2 Manter e utilizar medidas de segurança administrativas, técnicas e físicas apropriadas e suficientes para proteger a confidencialidade e integridade de todos os dados pessoais mantidos ou consultados/transmitidos eletronicamente, para garantir a proteção desses dados contra acesso não autorizado, destruição, uso, modificação, divulgação ou perda acidental ou indevida.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.3 Acessar os dados dentro de seu escopo e na medida abrangida por sua permissão de acesso (autorização) e que os dados pessoais não podem ser lidos, copiados, modificados ou removidos sem autorização expressa e por escrito da CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.4 Garantir, por si própria ou quaisquer de seus empregados, prepostos, sócios, diretores, representantes ou terceiros contratados, a confidencialidade dos dados processados, assegurando que todos estes que lidam com os dados pessoais sob responsabilidade da CONTRATANTE, assinaram Acordo de Confidencialidade com a CONTRATADA, bem como a manter quaisquer Dados Pessoais estritamente confidenciais e de não utilizá-los para outros fins, salvo para a fiel execução do objeto do contrato de prestação de serviço.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.5 A CONTRATADA em nenhuma hipótese repassará a terceiros informações relativas ao contrato de prestação de serviço firmado com a CONTRATANTE, salvo mediante requerimento expresso da CONTRATANTE, cuja autorização deverá, obrigatoriamente, conter o destinatário e os dados que serão objeto de compartilhamento. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.6 A CONTRATADA esclarece que os dados pessoais da CONTRATANTE ficarão em sua base de dados pelo período compreendido entre a data da assinatura do contrato, até 24 (vinte e quatro) meses após a entrega do produto/serviço contratado. Após essa data, todos os dados da CONTRATANTE serão destruídos/deletados do seu servidor; em caso de documento físico, estes serão, igualmente, destruídos.
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>10 - DO FORO</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>10.1 Fica eleito o foro da Comarca da instituição de Ensino, com renúncia expressa de qualquer outro, por mais privilegiado que seja, para solucionar todas as questões divergentes, resultantes da interpretação deste instrumento e, para nele, serem demandados a execução do cumprimento de todas as obrigações oriundas deste contrato, não obstante qualquer mudança de domicílio.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>10.2 Por estarem assim, justos e acordados, assinam o presente instrumento em 02 (duas) vias de igual teor na presença de 02 (duas) testemunhas para que surta os efeitos jurídicos e legais.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>Goiânia, <?php echo date('d'); ?> de <?php $mes = date('m');
                                                                                if ($mes == 1) {
                                                                                    echo "Janeiro";
                                                                                }
                                                                                if ($mes == 2) {
                                                                                    echo "Fevereiro";
                                                                                }
                                                                                if ($mes == 3) {
                                                                                    echo "Março";
                                                                                }
                                                                                if ($mes == 4) {
                                                                                    echo "Abril";
                                                                                }
                                                                                if ($mes == 5) {
                                                                                    echo "Maio";
                                                                                }
                                                                                if ($mes == 6) {
                                                                                    echo "Junho";
                                                                                }
                                                                                if ($mes == 7) {
                                                                                    echo "Julho";
                                                                                }
                                                                                if ($mes == 8) {
                                                                                    echo "Agosto";
                                                                                }
                                                                                if ($mes == 9) {
                                                                                    echo "Setembro";
                                                                                }
                                                                                if ($mes == 10) {
                                                                                    echo "Outubro";
                                                                                }
                                                                                if ($mes == 11) {
                                                                                    echo "Novembro";
                                                                                }
                                                                                if ($mes == 12) {
                                                                                    echo "Dezembro";
                                                                                } ?> de <?php echo date('Y'); ?>.</p>
                                        </td>
                                        </tr>

                                        </table>

                                        
                                        

                                        <table width="100%" BORDER="1" style="border-collapse: collapse">
                                        <tr>
                                        
                                        
                                        <td  style="text-align: center; width: 50%;">

                                        <!--<img style="width: 50%;" src="../imgs/assinaturas/Assinatura_Lanucia_CONTRATADA.png" alt="">-->
                                        Lanúcia Evangelista da Costa Moraes
                                        <br>        
                                        Data Aceite: <?= $dataaceite ?> 
                                        <br>
                                        Hora Aceite: <?= $horaatual ?>
                                        <br>
                                        IP de Acesso: <?= gethostbyname('www.studiomfotografia.com.br')?>
                                        <p>_______________________________</p>
                                        CONTRATADA
                                        </td>
                                        
                                        
                                        <td style="text-align: center; width: 50%;">
                                        <p>
                                        <?= $vetor_cadastro['nome']; ?>
                                        <br> 
                                        <?= $formando['email']  ?>
                                        <br> 
                                        Data Aceite: <?= $dataaceite ?> 
                                        <br>
                                        Hora Aceite: <?= $horaatual ?>
                                        <br>
                                        IP de Acesso: <?= $ip ?>
                                        <p>_______________________________</p>
                                        CONTRATANTE
                                        </td>
                                       
                                        </tr>
                                            
                                        </table>
                                        
                                        <table width="100%" BORDER="1" style="border-collapse: collapse">
                                            <tr>
                                                <td style="text-align: center; width: 50%;">
                                                    <img style="width: 50%;" src="../imgs/assinaturas/Assinatura_Andressa_TESTEMUNHA1.png" alt="">
                                                    <p>_______________________________</p>
                                                    TESTEMUNHA 1                               
                                                </td>
                                                
                                                <td style="text-align: center; width: 50%;">
                                                    <img style="width: 50%;" src="../imgs/assinaturas/Assinatura_Carol_TESTEMUNHA2.png" alt="">
                                                    <p>_______________________________</p>
                                                    TESTEMUNHA 2
                                                </td>
                                            </tr>
                                        </table>
                                                                            </textarea>
                                                                                    </div>
                                                                                <?php } ?>
                                                                            </div>
                                                                            <input type="text" name="id_item" value="<?php echo $id_item; ?>" hidden>
                                                                            <input type="text" name="formapag" value="<?php echo $formapag; ?>" hidden>
                                                                            <input type="text" name="diavencimento" value="<?php echo $diavencimento; ?>"
                                                                                hidden>
                                                                            <input type="text" name="qtdparcelas" value="<?php echo $qtdparcelas; ?>" hidden>

                                                                            <input id="license-check" type="checkbox" value="2" required="" onclick='return pergunta(this);'/>
                                                                            <label for="license-check">Concordo com os termos apresentados neste contrato.</label>

                                                                            <input id="btn-next" class="btn btn-success" type="submit" value="Avançar"/>

                                </form>
                                <?php }?>    
                                <?php if ($pacote['tipoContrato'] == 2) {
                                    # Contrato Taxa - Ensaio Foto de Convite
                                ?>

                                <form method="post" action="compra_assinatura_fotografia.php?id=<?= $id_item?>">
                                    <div id="license-box">
                                        <?php if ($pacote_itens['pacote_especial'] == '3') { ?>
                                            <div class="text">
                                    <textarea id="mymce" disabled="true">
                                        <table width="100%">
                                            <tr>
                                                <td width="50%"><img src="../imgs/logo1.png" width="100px"></td>
                                                <td width="50%" align="center">(62)3218-3476
                                                    <br>
                                                    Rua 93 nº 296 qd. F-14 lt. 36
                                                    <br>
                                                    St. Sul, Goiânia-GO - CEP 74083-120
                                                    <br>
                                                    cliente@studiomfotografia.com.br
                                                </td>
                                            </tr>
                                        </table>
                                        <div>
                                        <p>Parabéns por sua aquisição <?php echo $vetor_cadastro['nome']; ?></p>
                                        <p>Você faz parte do contrato: <?php echo $vetor_turma['ncontrato'] . ' - ' . $vetor_curso['nome'] . ' ' . $vetor_instituicao['nome'] . ' - '. $vetor_instituicao['cidade'] . ' - ' . $vetor_instituicao['estado']; ?></p>

                                        <p>Este é o extrato de sua compra:</p>

                                        <table width="50%">
                                            <tr>
                                                <th><strong><h3>Produto</h3></strong></th>
                                                <th><strong><h3>Quantidade</h3></strong></th>
                                                <th><strong><h3>Sub-Total</h3></strong></th>
                                            </tr>
                                            <tr>
                                                <td align="center">Fotos para Convite</td>
                                                <td align="center">01 un.</td>
                                                <td align="center">R$<?php echo number_format($pacote_itens['valor'], 2, ',', '.'); ?></td>
                                            </tr>
                                            <tr>
                                                <td align="center"></td>
                                                <td align="center"></td>
                                                <td align="center">Total: R$<?php echo number_format($pacote_itens['valor'], 2, ',', '.'); ?></td>
                                            </tr>
                                        </table>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Sua forma de pagamento escolhida é: <?php echo $vetor_forma['nome']; ?> em <?php echo $qtdparcelas; ?> parcela(s).</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">O produto “Fotos para Convite” consiste especificamente nas fotografias a serem produzidas para o convite de formatura, exceto as fotos de Família. Tais exemplos de fotografias estão especificados no documento de “Orientação Fotográfica”, advindo do convite de formatura.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Após a realização das fotos para o convite, as mesmas serão disponibilizadas para a escolha através da plataforma digital da empresa para que possam receber o (s) devido (s) tratamento (s) indicados por você. O prazo para que isto aconteça é de até 45 dias úteis e/ou prazo estipulado entre a comissão de formatura e a Studio M Fotografia.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">A entrega destas fotos tratadas também será através da plataforma digital logo após o tratamento das imagens.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Ficará a cargo da comissão de formatura a solicitação do agendamento dos dias para a realização do evento “Ensaio Foto de Convite”, o qual faz parte as “Foto para Convite”.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">O associado que, por qualquer motivo, cancelar a compra ou não participar das “Fotos para o Convite” não terá direito a ressarcimento, pois o mesmo fará ou fez parte do planejamento de gastos das “Fotos para Convite”.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Declaro para todos os fins de direito que, efetuei a compra das “Fotos para Convite” e estou ciente que o valor cobrado por tal produto está presente no contrato de fotografia, o qual rege todas as regras da prestação do serviço.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Concordo e aprovo todos os compromissos que a comissão de formatura venha assumir em meu nome para a realização para as “Fotos para Convite”, acatando as decisões e medidas por ela tomadas, e, por ser verdade, dato e confirmo a a compra das “Fotos para Convite”.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Assim sendo, na condição de contratante, ciente das minhas obrigações contratuais, firmo meu ACEITE na compra do produto “Fotos para Convite” o qual é parte integrante do contrato de fotografia.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Aceite da compra feito por meio de confirmação via sistema recebido pelo link de confirmação:</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">
                                            E-mail: <?php echo $vetor_cadastro['email']; ?> <br>
                                            Data Aceite: <br>
                                            Hora Aceite:  <br>
                                            IP de Acesso:  <br>
                                        </p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">
                                            Atenciosamente; <br>
                                            StudioM Fotografia
                                        </p>
                                    </textarea>
                                            </div>
                                        <?php }else { ?>
                                            <h1>Contrato Individual</h1>
                                            <div class="text">
                                    <textarea id="mymce2" disabled="true">
                                        <table width="100%">
                                            <tr>
                                                <td width="50%"><img src="data:image/png;base64," alt="" width="250px" /></td>
                                                <td width="50%">(62)3218-3476
                                                    <br>
                                        Rua 93 nº 296 qd. F-14 lt. 36
                                        <br>
                                        St. Sul, Goiânia-GO - CEP 74083-120
                                        <br>
                                        cliente@studiomfotografia.com.br
                                        </td>
                                            </tr>
                                        </table>
                                        <br>
                                        <div align="right">Contrato n° <?php echo $vetor_turma['ncontrato']; ?>></div>
                                        <p>
                                        </p>
                                        <p>
                                        </p>

                                        <p><strong><div align="center">CONTRATO INDIVIDUAL DE PRESTAÇÃO DE SERVIÇOS FOTOGRÁFICOS</div></strong></p>

                                        <p>
                                        </p>
                                        <table>

                                        <tr>
                                        <td>
                                        <p><strong>1 - PARTES CONTRATANTES</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>a) VENDEDORA CONTRATADA</strong>, doravante denominada simplesmente por Studio Med Fotografia e Turismo LTDA, pessoa jurídica de direito privado, inscrita no CNPJ sob o n° 24.917.717/0001-60, com sede na Rua 93 nº 296 qd. F-14 lt. 36, Cep: 74083-120, Setor Sul, Goiânia-Goiás.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>b) COMPRADOR CONTRATANTE</strong>, doravante designado apenas <strong>CONTRATANTE:</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>Nome:</strong> <?php echo $vetor_cadastro['nome']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>E-mail:</strong> <?php echo $vetor_cadastro['email']; ?> 
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>Endereço Completo:</strong> <?php echo $vetor_cadastro['endereco'] . '  ' .$vetor_cadastro['complemento']. '  ' . $vetor_cadastro['numero']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>Bairro:</strong> <?php echo $vetor_cadastro['bairro']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>Cidade:</strong> <?php echo $vetor_cadastro['cidade']; ?>   <strong>UF:</strong> <?php echo $vetor_cadastro['estado']; ?>   <strong>CEP:</strong> <?php echo $vetor_cadastro['cep']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>Telefone Resid.:</strong> <?php echo $vetor_cadastro['telefone']; ?>  <strong>Celular:</strong> <?php echo $vetor_cadastro['celular']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>CPF:</strong> <?php echo $vetor_cadastro['cpf']; ?> <strong>RG:</strong> <?php echo $vetor_cadastro['rg']; ?>    
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>2 - DO OBJETO DO CONTRATO</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>2.1 O presente instrumento de Contrato de Prestação de Serviços tem por objeto a comercialização de álbum de fotografia, nas condições abaixo estabelecidas.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>2.1.1 Compreende-se o produto álbum de fotografia o material fotográfico impresso e mídia digital disponibilizado pela CONTRATADA ao CONTRATANTE.</p>
                                        </td>
                                        </tr>
                                        <tr>
                                        <td>
                                        <p>2.2 integra ao objeto do presente contrato a prestação de serviços fotográficos para a realização de fotos para o convite de formatura, cujo serviço terá o preço adicional de <b>R$ <?= $pacote['valorContrato'] ?>(<?= extenso($pacote['valorContrato'])?>)</b>, incluso nesse valor, as fotografias indicadas em orientação fotográfica, provenientes do projeto do convite de formatura, exceto as fotos de família.</p>
                                        </td>
                                        </tr>
                                        <tr>
                                        <td>
                                        <p>2.2.1 A compra desse serviço deverá ser realizada através da plataforma digital (<span style="color: blue;text-decoration: underline;">www.studiomfotografia.com.br</span> - área do formando),  em até 30 dias antes da data de realização das fotos para o convite.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>3 – DOS EVENTOS CONTRATADOS</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <table width="40%" class="table table-bordered table-striped">
                                                <?php
                                                $sql_eventos = mysqli_query($con, "select * from eventos_pacote WHERE id_pacote = '$pacote_itens[id_item]' order by id_evento_pacote ASC");
                                                $i = 0;
                                                while ($vetor_eventos = mysqli_fetch_array($sql_eventos)) {
                                                    $sql_evento = mysqli_query($con, "select * from eventos_turma_lista where id_evento_turma = '$vetor_eventos[id_evento]'");
                                                    $vetor_evento = mysqli_fetch_array($sql_evento);
                                                    $sql_evento_nome = mysqli_query($con, "select * from categoriaevento where id_categoria = '$vetor_evento[id_evento]'");
                                                    $vetor_evento_nome = mysqli_fetch_array($sql_evento_nome);
                                        ?>
                                                    <tr>
                                                    <td><?php if ($vetor_evento_nome['nome'] == 'Pré-evento'){$i++; echo $vetor_evento_nome['nome'].' '.$i; }else { echo $vetor_evento_nome['nome']; }?></td>
                                                    </tr>
                                                <?php
                                                }
                                                ?>


                                        </table>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>4 - DOS PRODUTOS ADQUIRIDOS E PREÇO</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <table width="100%" class="table table-bordered table-striped">
                                        <tr style="    background: #000000; color: #fff;">
                                        <td width="5%">Item</td>
                                        <td width="95%">Produtos</td>
                                        </tr>

                                                                                <?php
                                                                            $sql_itens = mysqli_query($con, "select * from pacotes_itens_produtos WHERE id_pacote = '{$pacote_itens['id_item']}' order by id_produto_item ASC");
                                        $i = 1;

                                        while ($vetor_item=mysqli_fetch_array($sql_itens)) {

                                        $sql_produto = mysqli_query($con, "select * from tipo_opcionais where id_tipo = '{$vetor_item['id_produto']}'");
                                        $vetor_produto = mysqli_fetch_array($sql_produto);

                                        $valorun = number_format($vetor_item['valor'],2,',','.');
                                        ?>
                                        <tr>
                                        <td><?php echo $i; ?></td>
                                        <td><?php echo $vetor_produto['nome']; ?>

                                                                                <?php
                                        if($vetor_produto['id_tipo'] == 10 || $vetor_produto['id_tipo'] == 11 || $vetor_produto['id_tipo'] == 12 || $vetor_produto['id_tipo'] == 13 || $vetor_produto['id_tipo'] == 15) {

                                        echo $vetor_item['qtdpaginas'].' páginas</td>';

                                        }
                                        $i++;
                                        }
                                        ?>
                                        </table>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        4.1 Em razão dos produtos adquiridos, especificamente, pelo CONTRATANTE, as partes têm por certo e ajustado que o valor total dos produtos contratados é de R$ <?php echo $num = number_format($valorfinal, 2, ',', '.'); ?>(<?php echo strtoupper(Monetary::numberToExt($valorfinal)); ?>), considerando a quantidade de páginas do álbum de fotografia e produto (s) acima referenciado (s).
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>5 - FORMAS DE PAGAMENTO</strong>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>5.1 O Pagamento será realizado em <?php $valorparcela = $valorfinal / $qtdparcelas;
                                                                                echo $qtdparcelas; ?> parcela (s) de R$<?php echo $num = number_format($valorparcela, 2, ',', '.'); ?> (<?php echo strtoupper(Monetary::numberToExt($valorparcela)); ?>),  através da seguinte opção:</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        • <?php echo $vetor_forma['nome']; ?>.
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>

                                        <table width="100%" class="table table-bordered table-striped">

                                        <tr style="    background: #000000; color: #fff;">
                                        <td width="20%">Parcela</td>
                                        <td width="20%">Valor</td>
                                        <td width="60%">Data de Vencimento</td>
                                        </tr>
                                                                                <?php
                                                                                $i = 1;
                                                                                while ($i <= $qtdparcelas) {
                                                                                if ($formapag != '8') {
                                                                                    if ($formapag == '18') {
                                                                                        $data = date('Y-m-d', strtotime('+4 days'));
                                                                                    }else {
                                                                                        $data = date('Y-m', strtotime('+'.$i.' months'));
                                                                                    }
                                                                                }else {
                                                                                    $data = date('Y-m', strtotime('+'.$i.' months', strtotime(substr($pacote['dataentrega'], 0, 7))));
                                                                                }
                                                                                $dataexplode = explode('-', $data);
                                                                                $mesdup = $dataexplode[1];
                                                                                if ($mesdup == 1) {
                                                                                    $mesgerado1 = "Janeiro";
                                                                                }
                                                                                if ($mesdup == 2) {
                                                                                    $mesgerado1 = "Fevereiro";
                                                                                }
                                                                                if ($mesdup == 3) {
                                                                                    $mesgerado1 = "Março";
                                                                                }
                                                                                if ($mesdup == 4) {
                                                                                    $mesgerado1 = "Abril";
                                                                                }
                                                                                if ($mesdup == 5) {
                                                                                    $mesgerado1 = "Maio";
                                                                                }
                                                                                if ($mesdup == 6) {
                                                                                    $mesgerado1 = "Junho";
                                                                                }
                                                                                if ($mesdup == 7) {
                                                                                    $mesgerado1 = "Julho";
                                                                                }
                                                                                if ($mesdup == 8) {
                                                                                    $mesgerado1 = "Agosto";
                                                                                }
                                                                                if ($mesdup == 9) {
                                                                                    $mesgerado1 = "Setembro";
                                                                                }
                                                                                if ($mesdup == 10) {
                                                                                    $mesgerado1 = "Outubro";
                                                                                }
                                                                                if ($mesdup == 11) {
                                                                                    $mesgerado1 = "Novembro";
                                                                                }
                                                                                if ($mesdup == 12) {
                                                                                    $mesgerado1 = "Dezembro";
                                                                                }
                                                                                $dataduplicata = $mesgerado1.'/'.$dataexplode[0];
                                                                                ?>
                                        <tr>
                                        <td><?php echo $i.'/'.$qtdparcelas; ?></td>
                                        <td>R$ <?php echo $num = number_format($valorparcela, 2, ',', '.'); ?></td>
                                        <td><?php echo $dataduplicata; ?></td>
                                        </tr>
                                        <?php
                                            $i++;
                                        }
                                        ?>
                                        </table>

                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>6 - DO INADIMPLEMENTO DAS PARCELAS E OBRIGAÇÕES</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>6.1 O não pagamento de qualquer parcela acima ajustado na data prevista caracterizará a mora da <strong>CONTRATANTE</strong> e, implicará no vencimento antecipado da dívida, independente de qualquer notificação do <strong>CONTRATANTE</strong>, com a incidência de multa de 2% (dois por cento), sobre a dívida original, acrescido de juros à taxa de 1% (um por cento) ao mês, bem como atualização monetária pelo INPC - Índice Nacional Preço ao Consumidor, além das despesas judiciais e dos honorários advocatórios aqui estabelecidos entre as partes em 20% (vinte por cento), a serem pagos pela <strong>CONTRATANTE.</strong> </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>6.2 O presente instrumento poderá ser rescindido a qualquer tempo, desde que seja expressamente notificada a outra parte com 30 (trinta) dias de antecedência, sem prejuízo de aplicação das multas previstas nos itens seguintes. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>6.2.1 À parte (CONTRATANTE ou CONTRATADA) que rescindir voluntariamente o contrato, ficará sujeita a multa rescisória equivalente a 10% (dez por cento) sobre o valor dos <strong> eventos não realizados</strong>, listados no item 3 deste instrumento.  </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>6.2.2 Em caso de rescisão pela CONTRATANTE, essa ficará sujeita a restituir a CONTRATADA, toda e qualquer bonificação recebida durante a vigência do presente instrumento. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>6.3 As penalidades previstas nos itens <strong>6.2.1</strong> e <strong>6.2.2</strong> não afastará a obrigação da CONTRATANTE de efetuar o pagamento do (s) evento (s) executado (s), conforme item 3 deste instrumento.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>6.4 Para fim de aplicação da multa ajustada nos “itens <strong>6.2.1</strong> e <strong>6.2.2</strong>”, a parte deverá <strong>comunicar expressamente a outra</strong>, o descumprimento da eventual obrigação prevista neste instrumento, ocasião que, fica ajustado o prazo de 10 (dez) dias úteis para eventual esclarecimento ou, se for o caso, cumprimento da obrigação.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>6.5 <strong>A multa prevista no item 6.2.1</strong> não será aplicada ao CONTRATANTE que por ventura seja impedido de acompanhar a respectiva turma de formatura, em razão de transferência de instituição ou em caso de reprovação. Entretanto, ficará sujeito ao pagamento de todos os serviços prestados pela CONTRATADA e já entregue ao CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>7 - DA ENTREGA DO PRODUTO</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1 É obrigação da CONTRATADA entregar os produtos adquiridos pelo CONTRATANTE dentro dos prazos estipulados e nas seguintes condições: </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.1 As fotos serão disponibilizadas para a escolha e aprovação do (s) produto (s) <b>exclusivamente</b> pela plataforma digital (<span style="color: blue;text-decoration: underline">www.studiomfotografia.com.br</span> - área do formando); </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.1.1 Em até 45 dias úteis após a data do último evento da turma as fotos serão disponibilizadas para a escolha através da plataforma digital <b>(7.1.1)</b>. Em conjunto com as fotografias, será (ão) disponibilizado (s) o (s) mostruário (s) piloto do (s) produto (s) no intuito de auxiliar a todos na escolha das fotos.
                                        </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.1.2 A partir da escolha das fotos pelo CONTRATANTE, a CONTRATADA terá o prazo de até 30 dias úteis, para disponibilizar o (s) produto (s) diagramado (s) para a APROVAÇÃO e, se for o caso, a RETIFICAÇÃO solicitada pelo CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.1.3 O (s) produto (s) contratado (s) somente serão produzidos mediante a aprovação do CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.2 A partir da aprovação do (s) produto (s) pelo CONTRATANTE, a CONTRATADA terá o prazo de até 45 dias úteis, para o envio do (s) produto (s) para o endereço indicado pelo CONTRATANTE neste instrumento.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.2.1 A CONTRATADA não será, em nenhuma hipótese, responsável pelo extravio do (s) produto (s) contratado (s) por erro/incorreção no endereço de destino da mercadoria.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.3 Caso o produto fique pendente de escolha <strong>(7.1.1.1)</strong> e aprovação <strong>(7.1.1.2)</strong> pelo CONTRATANTE, a CONTRATADA não será responsabilizada por qualquer atraso na entrega do (s) produto (s) contratado (s).</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>8 - DAS CONDIÇÕES GERAIS</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.1 As datas dos eventos extraoficiais e oficiais, incluindo a data para a realização das fotos para o convite de formatura serão informadas previamente pela comissão de formatura, não tendo o CONTRATANTE e CONTRATADA qualquer responsabilidade sobre o agendamento dos mencionados eventos.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.2 <b>As fotos para o convite não serão realizadas em outra data que não seja a data oficial informada pela comissão de formatura, exceto em casos excepcionais, tais como luto, acidente ou outra situação agravante que comprometa o comparecimento ao evento, desde que possa ser comprovado. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.2.1 Caso ocorra a hipótese prevista no item anterior, o CONTRATANTE deverá reagendar a realização das fotos em data de evento disponível pela CONTRATADA, desde que não comprometa o prazo de entrega das fotos para a empresa responsável pela confecção do convite de formatura.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.2.2 Eventual indisponibilidade de data e evento para que sejam realizadas tais fotografias, será de inteira responsabilidade do formando fazê-las por conta própria e enviá-las à empresa responsável pela confecção do convite de formatura, sendo certo que a CONTRATANTE não se responsabilizará por qualquer serviço realizado por terceiros.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.2.3 Caso haja a necessidade de deslocamento entre cidades para a realização das fotografias para o convite, esta será de inteira responsabilidade do CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.3 <b>A CONTRATANTE autoriza expressamente e sem qualquer ônus, a utilização das imagens</b> dos eventos de formatura, para efeitos de divulgação da CONTRATADA, por parte da CONTRATADA.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.4 <b>Fica autorizada a CONTRATADA</b> posicionar sua logomarca, a título de identificação de sua autoria do (s) produto (s) e/ou serviço (s) que com ela forem contratados.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.5 Na hipótese de o CONTRATANTE ficar inadimplente com as obrigações financeiras, os produtos adquiridos por este contrato ficarão retidos até a resolução de tais pendências.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.6 AS PARTES, devidamente nomeadas e qualificadas no preâmbulo deste instrumento, responsabilizam-se solidariamente, em caráter irrevogável e irretratável, pelo cumprimento deste contrato.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.7 A obrigação ora reconhecida e assumida pelos CONTRATANTES, como líquida, certa e exigível, no valor  mencionado no <b>item 4.1</b>, aplica-se o disposto no artigo 784 ,III do Código de Processo Civil, vez que possui força de <b>Título Executivo Extrajudicial</b>.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>9 – DA PROTEÇÃO DE DADOS PESSOAIS</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1 A CONTRATADA, por si e por seus colaboradores, obriga- se a atuar no presente Contrato em conformidade com a Legislação vigente sobre Proteção de Dados Pessoais e as determinações de órgãos reguladores/fiscalizadores sobre a matéria, em especial a Lei 13.709/2018, além das demais normas e políticas de proteção de dados de cada país onde houver qualquer tipo de tratamento dos dados dos clientes, o que inclui os dados dos clientes desta. No manuseio dos dados a CONTRATADA deverá:</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.1 Tratar os dados pessoais a que tiver acesso apenas de acordo com as instruções da CONTRATANTE e em conformidade com estas cláusulas, e que, na eventualidade, de não mais poder cumprir estas obrigações, por qualquer razão, concorda em informar de modo formal este fato imediatamente à CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.2 Manter e utilizar medidas de segurança administrativas, técnicas e físicas apropriadas e suficientes para proteger a confidencialidade e integridade de todos os dados pessoais mantidos ou consultados/transmitidos eletronicamente, para garantir a proteção desses dados contra acesso não autorizado, destruição, uso, modificação, divulgação ou perda acidental ou indevida.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.3 Acessar os dados dentro de seu escopo e na medida abrangida por sua permissão de acesso (autorização) e que os dados pessoais não podem ser lidos, copiados, modificados ou removidos sem autorização expressa e por escrito da CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.4 Garantir, por si própria ou quaisquer de seus empregados, prepostos, sócios, diretores, representantes ou terceiros contratados, a confidencialidade dos dados processados, assegurando que todos estes que lidam com os dados pessoais sob responsabilidade da CONTRATANTE, assinaram Acordo de Confidencialidade com a CONTRATADA, bem como a manter quaisquer Dados Pessoais estritamente confidenciais e de não utilizá-los para outros fins, salvo para a fiel execução do objeto do contrato de prestação de serviço.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.5 A CONTRATADA em nenhuma hipótese repassará a terceiros informações relativas ao contrato de prestação de serviço firmado com a CONTRATANTE, salvo mediante requerimento expresso da CONTRATANTE, cuja autorização deverá, obrigatoriamente, conter o destinatário e os dados que serão objeto de compartilhamento.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.6 A CONTRATADA esclarece que os dados pessoais da CONTRATANTE ficarão em sua base de dados pelo período compreendido entre a data da assinatura do contrato, até 24 (vinte e quatro) meses após a entrega do produto/serviço contratado. Após essa data, todos os dados da CONTRATANTE serão destruídos/deletados do seu servidor; em caso de documento físico, estes serão, igualmente, destruídos.  </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>10 - DO FORO</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>10.1 Fica eleito o foro da Comarca da instituição de Ensino, com renúncia expressa de qualquer outro, por mais privilegiado que seja, para solucionar todas as questões divergentes, resultantes da interpretação deste instrumento e, para nele, serem demandados a execução do cumprimento de todas as obrigações oriundas deste contrato, não obstante qualquer mudança de domicílio.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>10.2 Por estarem assim, justos e acordados, assinam o presente instrumento em 02 (duas) vias de igual teor na presença de 02 (duas) testemunhas para que surta os efeitos jurídicos e legais.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>Goiânia, <?php echo date('d'); ?> de <?php $mes = date('m');
                                                                                if ($mes == 1) {
                                                                                    echo "Janeiro";
                                                                                }
                                                                                if ($mes == 2) {
                                                                                    echo "Fevereiro";
                                                                                }
                                                                                if ($mes == 3) {
                                                                                    echo "Março";
                                                                                }
                                                                                if ($mes == 4) {
                                                                                    echo "Abril";
                                                                                }
                                                                                if ($mes == 5) {
                                                                                    echo "Maio";
                                                                                }
                                                                                if ($mes == 6) {
                                                                                    echo "Junho";
                                                                                }
                                                                                if ($mes == 7) {
                                                                                    echo "Julho";
                                                                                }
                                                                                if ($mes == 8) {
                                                                                    echo "Agosto";
                                                                                }
                                                                                if ($mes == 9) {
                                                                                    echo "Setembro";
                                                                                }
                                                                                if ($mes == 10) {
                                                                                    echo "Outubro";
                                                                                }
                                                                                if ($mes == 11) {
                                                                                    echo "Novembro";
                                                                                }
                                                                                if ($mes == 12) {
                                                                                    echo "Dezembro";
                                                                                } ?> de <?php echo date('Y'); ?>.</p>
                                        </td>
                                        </tr>

                                        </table>


                                        <table width="100%" BORDER="1" style="border-collapse: collapse">
                                        <tr>
                                        
                                        
                                        <td  style="text-align: center; width: 50%;">

                                        <!--<img style="width: 50%;" src="../imgs/assinaturas/Assinatura_Lanucia_CONTRATADA.png" alt="">-->
                                        Lanúcia Evangelista da Costa Moraes
                                        <br>        
                                        Data Aceite: <?= $dataaceite ?> 
                                        <br>
                                        Hora Aceite: <?= $horaatual ?>
                                        <br>
                                        IP de Acesso: <?= gethostbyname('www.studiomfotografia.com.br')?>
                                        <p>_______________________________</p>
                                        CONTRATADA
                                        </td>
                                        
                                        
                                        <td style="text-align: center; width: 50%;">
                                        <p>
                                        <?= $vetor_cadastro['nome']; ?>
                                        <br> 
                                        <?= $formando['email']  ?>
                                        <br> 
                                        Data Aceite: <?= $dataaceite ?> 
                                        <br>
                                        Hora Aceite: <?= $horaatual ?>
                                        <br>
                                        IP de Acesso: <?= $ip ?>
                                        <p>_______________________________</p>
                                        CONTRATANTE
                                        </td>
                                       
                                        </tr>
                                            
                                        </table>
                                        
                                        <table width="100%" BORDER="1" style="border-collapse: collapse">
                                            <tr>
                                                <td style="text-align: center; width: 50%;">
                                                    <img style="width: 50%;" src="../imgs/assinaturas/Assinatura_Andressa_TESTEMUNHA1.png" alt="">
                                                    <p>_______________________________</p>
                                                    TESTEMUNHA 1                               
                                                </td>
                                                
                                                <td style="text-align: center; width: 50%;">
                                                    <img style="width: 50%;" src="../imgs/assinaturas/Assinatura_Carol_TESTEMUNHA2.png" alt="">
                                                    <p>_______________________________</p>
                                                    TESTEMUNHA 2
                                                </td>
                                            </tr>
                                        </table>
                                                                            </textarea>
                                                                                    </div>
                                                                                <?php } ?>
                                                                            </div>
                                                                            <input type="text" name="id_item" value="<?php echo $id_item; ?>" hidden>
                                                                            <input type="text" name="formapag" value="<?php echo $formapag; ?>" hidden>
                                                                            <input type="text" name="diavencimento" value="<?php echo $diavencimento; ?>"
                                                                                hidden>
                                                                            <input type="text" name="qtdparcelas" value="<?php echo $qtdparcelas; ?>" hidden>

                                                                            <input id="license-check" type="checkbox" value="2" required="" onclick='return pergunta(this);'/>
                                                                            <label for="license-check">Concordo com os termos apresentados neste contrato.</label>

                                                                            <input id="btn-next" class="btn btn-success" type="submit" value="Avançar"/>

                                </form>
                                <?php }?>    
                                   
                                <?php if ($pacote['tipoContrato'] == 3) {
                                    # Contrato Página - Ensaio Foto de Convite
                                ?>

                                <form method="post" action="compra_assinatura_fotografia.php?id=<?= $id_item?>">
                                    <div id="license-box">
                                        <?php if ($pacote_itens['pacote_especial'] == '3') { ?>
                                            <div class="text">
                                    <textarea id="mymce" disabled="true">
                                        <table width="100%">
                                            <tr>
                                                <td width="50%"><img src="../imgs/logo1.png" width="100px"></td>
                                                <td width="50%" align="center">(62)3218-3476
                                                    <br>
                                                    Rua 93 nº 296 qd. F-14 lt. 36
                                                    <br>
                                                    St. Sul, Goiânia-GO - CEP 74083-120
                                                    <br>
                                                    cliente@studiomfotografia.com.br
                                                </td>
                                            </tr>
                                        </table>
                                        <div>
                                        <p>Parabéns por sua aquisição <?php echo $vetor_cadastro['nome']; ?></p>
                                        <p>Você faz parte do contrato: <?php echo $vetor_turma['ncontrato'] . ' - ' . $vetor_curso['nome'] . ' ' . $vetor_instituicao['nome'] . ' - '. $vetor_instituicao['cidade'] . ' - ' . $vetor_instituicao['estado']; ?></p>

                                        <p>Este é o extrato de sua compra:</p>

                                        <table width="50%">
                                            <tr>
                                                <th><strong><h3>Produto</h3></strong></th>
                                                <th><strong><h3>Quantidade</h3></strong></th>
                                                <th><strong><h3>Sub-Total</h3></strong></th>
                                            </tr>
                                            <tr>
                                                <td align="center">Fotos para Convite</td>
                                                <td align="center">01 un.</td>
                                                <td align="center">R$<?php echo number_format($pacote_itens['valor'], 2, ',', '.'); ?></td>
                                            </tr>
                                            <tr>
                                                <td align="center"></td>
                                                <td align="center"></td>
                                                <td align="center">Total: R$<?php echo number_format($pacote_itens['valor'], 2, ',', '.'); ?></td>
                                            </tr>
                                        </table>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Sua forma de pagamento escolhida é: <?php echo $vetor_forma['nome']; ?> em <?php echo $qtdparcelas; ?> parcela(s).</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">O produto “Fotos para Convite” consiste especificamente nas fotografias a serem produzidas para o convite de formatura, exceto as fotos de Família. Tais exemplos de fotografias estão especificados no documento de “Orientação Fotográfica”, advindo do convite de formatura.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Após a realização das fotos para o convite, as mesmas serão disponibilizadas para a escolha através da plataforma digital da empresa para que possam receber o (s) devido (s) tratamento (s) indicados por você. O prazo para que isto aconteça é de até 45 dias úteis e/ou prazo estipulado entre a comissão de formatura e a Studio M Fotografia.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">A entrega destas fotos tratadas também será através da plataforma digital logo após o tratamento das imagens.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Ficará a cargo da comissão de formatura a solicitação do agendamento dos dias para a realização do evento “Ensaio Foto de Convite”, o qual faz parte as “Foto para Convite”.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">O associado que, por qualquer motivo, cancelar a compra ou não participar das “Fotos para o Convite” não terá direito a ressarcimento, pois o mesmo fará ou fez parte do planejamento de gastos das “Fotos para Convite”.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Declaro para todos os fins de direito que, efetuei a compra das “Fotos para Convite” e estou ciente que o valor cobrado por tal produto está presente no contrato de fotografia, o qual rege todas as regras da prestação do serviço.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Concordo e aprovo todos os compromissos que a comissão de formatura venha assumir em meu nome para a realização para as “Fotos para Convite”, acatando as decisões e medidas por ela tomadas, e, por ser verdade, dato e confirmo a a compra das “Fotos para Convite”.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Assim sendo, na condição de contratante, ciente das minhas obrigações contratuais, firmo meu ACEITE na compra do produto “Fotos para Convite” o qual é parte integrante do contrato de fotografia.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Aceite da compra feito por meio de confirmação via sistema recebido pelo link de confirmação:</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">
                                            E-mail: <?php echo $vetor_cadastro['email']; ?> <br>
                                            Data Aceite: <br>
                                            Hora Aceite:  <br>
                                            IP de Acesso:  <br>
                                        </p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">
                                            Atenciosamente; <br>
                                            StudioM Fotografia
                                        </p>
                                    </textarea>
                                            </div>
                                        <?php }else { ?>
                                            <h1>Contrato Individual</h1>
                                            <div class="text">
                                    <textarea id="mymce2" disabled="true">
                                        <table width="100%">
                                            <tr>
                                                <td width="50%"><img src="data:image/png;base64," alt="" width="250px" /></td>
                                                <td width="50%">(62)3218-3476
                                                    <br>
                                        Rua 93 nº 296 qd. F-14 lt. 36
                                        <br>
                                        St. Sul, Goiânia-GO - CEP 74083-120
                                        <br>
                                        cliente@studiomfotografia.com.br
                                        </td>
                                            </tr>
                                        </table>
                                        <br>
                                        <div align="right">Contrato n° <?php echo $vetor_turma['ncontrato']; ?>></div>
                                        <p>
                                        </p>
                                        <p>
                                        </p>

                                        <p><strong><div align="center">CONTRATO INDIVIDUAL DE PRESTAÇÃO DE SERVIÇOS FOTOGRÁFICOS</div></strong></p>

                                        <p>
                                        </p>
                                        <table>

                                        <tr>
                                        <td>
                                        <p><strong>1 - PARTES CONTRATANTES</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>a) VENDEDORA CONTRATADA</strong>, doravante denominada simplesmente por Studio Med Fotografia e Turismo LTDA, pessoa jurídica de direito privado, inscrita no CNPJ sob o n° 24.917.717/0001-60, com sede na Rua 93 nº 296 qd. F-14 lt. 36, Cep: 74083-120, Setor Sul, Goiânia-Goiás.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>b) COMPRADOR CONTRATANTE</strong>, doravante designado apenas <strong>CONTRATANTE:</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>Nome:</strong> <?php echo $vetor_cadastro['nome']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>E-mail:</strong> <?php echo $vetor_cadastro['email']; ?> 
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>Endereço Completo:</strong> <?php echo $vetor_cadastro['endereco'] . '  ' .$vetor_cadastro['complemento']. '  ' . $vetor_cadastro['numero']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>Bairro:</strong> <?php echo $vetor_cadastro['bairro']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>Cidade:</strong> <?php echo $vetor_cadastro['cidade']; ?>   <strong>UF:</strong> <?php echo $vetor_cadastro['estado']; ?>   <strong>CEP:</strong> <?php echo $vetor_cadastro['cep']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>Telefone Resid.:</strong> <?php echo $vetor_cadastro['telefone']; ?>  <strong>Celular:</strong> <?php echo $vetor_cadastro['celular']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>CPF:</strong> <?php echo $vetor_cadastro['cpf']; ?> <strong>RG:</strong> <?php echo $vetor_cadastro['rg']; ?>    
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>2 - DO OBJETO DO CONTRATO</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>2.1 O presente instrumento de Contrato de Prestação de Serviços tem por objeto a comercialização do álbum de fotográfico, nas condições abaixo estabelecidas.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>2.1.1 Compreende-se o produto <b>álbum fotográfico</b> o material fotográfico impresso e mídia digital disponibilizado pela CONTRATADA ao CONTRATANTE. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>3 – DOS EVENTOS CONTRATADOS</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <table width="40%" class="table table-bordered table-striped">
                                                <?php
                                                $sql_eventos = mysqli_query($con, "select * from eventos_pacote WHERE id_pacote = '$pacote_itens[id_item]' order by id_evento_pacote ASC");
                                                $i = 0;
                                                while ($vetor_eventos = mysqli_fetch_array($sql_eventos)) {
                                                    $sql_evento = mysqli_query($con, "select * from eventos_turma_lista where id_evento_turma = '$vetor_eventos[id_evento]'");
                                                    $vetor_evento = mysqli_fetch_array($sql_evento);
                                                    $sql_evento_nome = mysqli_query($con, "select * from categoriaevento where id_categoria = '$vetor_evento[id_evento]'");
                                                    $vetor_evento_nome = mysqli_fetch_array($sql_evento_nome);
                                        ?>
                                                    <tr>
                                                    <td><?php if ($vetor_evento_nome['nome'] == 'Pré-evento'){$i++; echo $vetor_evento_nome['nome'].' '.$i; }else { echo $vetor_evento_nome['nome']; }?></td>
                                                    </tr>
                                                <?php
                                                }
                                                ?>


                                        </table>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>3.1 Ensaio Foto de Convite</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>3.1.1 A CONTRATANTE, 12 (doze) meses antes da formatura, deverá efetuar o pagamento do Ensaio Foto de Convite no valor de <b>R$ <?= $pacote['valorContrato'] ?>(<?= extenso($pacote['valorContrato'])?>)</b>, cujo preço corresponde às 10 (dez) páginas do álbum fotográfico.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>3.1.2 No Ensaio Foto de Convite estão inclusas as fotografias indicadas em orientação fotográfica, provenientes do projeto do convite de formatura, exceto as fotos de família.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>4 - DOS PRODUTOS ADQUIRIDOS E PREÇO</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <table width="100%" class="table table-bordered table-striped">
                                        <tr style="    background: #000000; color: #fff;">
                                        <td width="5%">Item</td>
                                        <td width="95%">Produtos</td>
                                        </tr>

                                                                                <?php
                                                                            $sql_itens = mysqli_query($con, "select * from pacotes_itens_produtos WHERE id_pacote = '{$pacote_itens['id_item']}' order by id_produto_item ASC");
                                        $i = 1;

                                        while ($vetor_item=mysqli_fetch_array($sql_itens)) {

                                        $sql_produto = mysqli_query($con, "select * from tipo_opcionais where id_tipo = '{$vetor_item['id_produto']}'");
                                        $vetor_produto = mysqli_fetch_array($sql_produto);

                                        $valorun = number_format($vetor_item['valor'],2,',','.');
                                        ?>
                                        <tr>
                                        <td><?php echo $i; ?></td>
                                        <td><?php echo $vetor_produto['nome']; ?>

                                                                                <?php
                                        if($vetor_produto['id_tipo'] == 10 || $vetor_produto['id_tipo'] == 11 || $vetor_produto['id_tipo'] == 12 || $vetor_produto['id_tipo'] == 13 || $vetor_produto['id_tipo'] == 15) {

                                        echo $vetor_item['qtdpaginas'].' páginas</td>';

                                        }
                                        $i++;
                                        }
                                        ?>
                                        </table>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        4.1 Em razão dos produtos adquiridos, especificamente, pelo CONTRATANTE, as partes têm por certo e ajustado que o valor total dos produtos contratados é de R$ <?php echo $num = number_format($valorfinal, 2, ',', '.'); ?>(<?php echo strtoupper(Monetary::numberToExt($valorfinal)); ?>), considerando a quantidade de páginas do álbum de fotografia e produto (s) acima referenciado (s).
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>5 - FORMAS DE PAGAMENTO</strong>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>5.1 O Pagamento será realizado em <?php $valorparcela = $valorfinal / $qtdparcelas;
                                                                                echo $qtdparcelas; ?> parcela (s) de R$<?php echo $num = number_format($valorparcela, 2, ',', '.'); ?> (<?php echo strtoupper(Monetary::numberToExt($valorparcela)); ?>),  através da seguinte opção:</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        • <?php echo $vetor_forma['nome']; ?>.
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>

                                        <table width="100%" class="table table-bordered table-striped">

                                        <tr style="    background: #000000; color: #fff;">
                                        <td width="20%">Parcela</td>
                                        <td width="20%">Valor</td>
                                        <td width="60%">Data de Vencimento</td>
                                        </tr>
                                                                                <?php
                                                                                $i = 1;
                                                                                while ($i <= $qtdparcelas) {
                                                                                if ($formapag != '8') {
                                                                                    if ($formapag == '18') {
                                                                                        $data = date('Y-m-d', strtotime('+4 days'));
                                                                                    }else {
                                                                                        $data = date('Y-m', strtotime('+'.$i.' months'));
                                                                                    }
                                                                                }else {
                                                                                    $data = date('Y-m', strtotime('+'.$i.' months', strtotime(substr($pacote['dataentrega'], 0, 7))));
                                                                                }
                                                                                $dataexplode = explode('-', $data);
                                                                                $mesdup = $dataexplode[1];
                                                                                if ($mesdup == 1) {
                                                                                    $mesgerado1 = "Janeiro";
                                                                                }
                                                                                if ($mesdup == 2) {
                                                                                    $mesgerado1 = "Fevereiro";
                                                                                }
                                                                                if ($mesdup == 3) {
                                                                                    $mesgerado1 = "Março";
                                                                                }
                                                                                if ($mesdup == 4) {
                                                                                    $mesgerado1 = "Abril";
                                                                                }
                                                                                if ($mesdup == 5) {
                                                                                    $mesgerado1 = "Maio";
                                                                                }
                                                                                if ($mesdup == 6) {
                                                                                    $mesgerado1 = "Junho";
                                                                                }
                                                                                if ($mesdup == 7) {
                                                                                    $mesgerado1 = "Julho";
                                                                                }
                                                                                if ($mesdup == 8) {
                                                                                    $mesgerado1 = "Agosto";
                                                                                }
                                                                                if ($mesdup == 9) {
                                                                                    $mesgerado1 = "Setembro";
                                                                                }
                                                                                if ($mesdup == 10) {
                                                                                    $mesgerado1 = "Outubro";
                                                                                }
                                                                                if ($mesdup == 11) {
                                                                                    $mesgerado1 = "Novembro";
                                                                                }
                                                                                if ($mesdup == 12) {
                                                                                    $mesgerado1 = "Dezembro";
                                                                                }
                                                                                $dataduplicata = $mesgerado1.'/'.$dataexplode[0];
                                                                                ?>
                                        <tr>
                                        <td><?php echo $i.'/'.$qtdparcelas; ?></td>
                                        <td>R$ <?php echo $num = number_format($valorparcela, 2, ',', '.'); ?></td>
                                        <td><?php echo $dataduplicata; ?></td>
                                        </tr>
                                        <?php
                                            $i++;
                                        }
                                        ?>
                                        </table>

                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>6 - DO INADIMPLEMENTO DAS PARCELAS E OBRIGAÇÕES</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>6.1 O não pagamento de qualquer parcela na data prevista caracterizará a mora da <b>CONTRATANTE</b> e, implicará no vencimento antecipado da dívida, independente de qualquer notificação do <b>CONTRATANTE</b>, com a incidência de multa de 2% (dois por cento), sobre a dívida original, acrescido de juros à taxa de 1% (um por cento) ao mês, bem como atualização monetária pelo INPC - Índice Nacional Preço ao Consumidor, além das despesas judiciais e dos honorários advocatórios aqui estabelecidos entre as partes em 20% (vinte por cento), a serem pagos pela <b>CONTRATANTE</b>. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>6.2 O presente instrumento poderá ser rescindido a qualquer tempo, desde que seja expressamente notificada a outra parte com 30 (trinta) dias de antecedência, sem prejuízo de aplicação das multas previstas nos itens seguintes. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>6.2.1 À parte (CONTRATANTE ou CONTRATADA) que rescindir voluntariamente o contrato, ficará sujeita a multa rescisória equivalente a 10% (dez por cento) sobre o valor dos eventos não realizados, listados no item 3 deste instrumento. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>6.2.2 Em caso de rescisão pela CONTRATANTE, essa ficará sujeita a restituir a CONTRATADA, toda e qualquer bonificação recebida durante a vigência do presente instrumento.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>6.3 As penalidades previstas nos itens <strong>6.2.1</strong> e <strong>6.2.2</strong> não afastará a obrigação da CONTRATANTE de efetuar o pagamento do (s) evento (s) executado (s), conforme item 3 deste instrumento.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>6.4 Para fim de aplicação da multa ajustada nos “itens <strong>6.2.1</strong> e <strong>6.2.2</strong>”, a parte deverá <strong>comunicar expressamente a outra</strong>, o descumprimento da eventual obrigação prevista neste instrumento, ocasião que, fica ajustado o prazo de 10 (dez) dias úteis para eventual esclarecimento ou, se for o caso, cumprimento da obrigação.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>6.5 <strong>A multa prevista no item 6.2.1</strong> não será aplicada ao CONTRATANTE que por ventura seja impedido de acompanhar a respectiva turma de formatura, em razão de transferência de instituição ou em caso de reprovação. Entretanto, ficará sujeito ao pagamento de todos os serviços prestados pela CONTRATADA e já entregue ao CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>7 - DA ENTREGA DO PRODUTO</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1 É obrigação da CONTRATADA entregar os produtos adquiridos pelo CONTRATANTE dentro dos prazos estipulados e nas seguintes condições: </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.1 As fotos serão disponibilizadas para a escolha e aprovação do (s) produto (s) <b>exclusivamente</b> pela plataforma digital (<span style="color: blue;text-decoration: underline">www.studiomfotografia.com.br</span> - área do formando); </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.1.1 Em até 45 dias úteis após a data do último evento da turma as fotos serão disponibilizadas para a escolha através da plataforma digital <b>(7.1.1)</b>. Em conjunto com as fotografias, será (ão) disponibilizado (s) o (s) mostruário (s) piloto do (s) produto (s) no intuito de auxiliar a todos na escolha das fotos.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.1.2 A partir da escolha das fotos pelo CONTRATANTE, a CONTRATADA terá o prazo de até 30 dias úteis, para disponibilizar o (s) produto (s) diagramado (s) para a APROVAÇÃO e, se for o caso, a RETIFICAÇÃO solicitada pelo CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.1.3 O (s) produto (s) contratado (s) somente serão produzidos mediante a aprovação do CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.2 A partir da aprovação do (s) produto (s) pelo CONTRATANTE, a CONTRATADA terá o prazo de até 45 dias úteis, para o envio do (s) produto (s) para o endereço indicado pelo CONTRATANTE neste instrumento.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.1.2.1 A CONTRATADA não será, em nenhuma hipótese, responsável pelo extravio do (s) produto (s) contratado (s) por erro/incorreção no endereço de destino da mercadoria.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>7.3 Caso o produto fique pendente de escolha <strong>(7.1.1.1)</strong> e aprovação <strong>(7.1.1.2)</strong> pelo CONTRATANTE, a CONTRATADA não será responsabilizada por qualquer atraso na entrega do (s) produto (s) contratado (s).</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>8 - DAS CONDIÇÕES GERAIS</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.1 As datas dos eventos extraoficiais e oficiais, incluindo a data para a realização das fotos para o convite de formatura serão informadas previamente pela comissão de formatura, não tendo o CONTRATANTE e CONTRATADA qualquer responsabilidade sobre o agendamento dos mencionados eventos. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.2 <b>As fotos para o convite não serão realizadas em outra data que não seja a data oficial informada pela comissão de formatura, exceto em casos excepcionais, tais como </b> luto, acidente ou outra situação agravante que comprometa o comparecimento ao evento, desde que possa ser comprovado. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.2.1 Caso ocorra a hipótese prevista no item anterior, o CONTRATANTE deverá reagendar a realização das fotos em data de evento disponível pela CONTRATADA, desde que não comprometa o prazo de entrega das fotos para a empresa responsável pela confecção do convite de formatura.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.2.2 Eventual indisponibilidade de data e evento para que sejam realizadas tais fotografias, será de inteira responsabilidade do formando fazê-las por conta própria e enviá-las à empresa responsável pela confecção do convite de formatura, sendo certo que a CONTRATANTE não se responsabilizará por qualquer serviço realizado por terceiros.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.2.3 Caso haja a necessidade de deslocamento entre cidades para a realização das fotografias para o convite, esta será de inteira responsabilidade do CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.3 <b>A CONTRATANTE autoriza expressamente e sem qualquer ônus, a utilização das imagens</b> dos eventos de formatura, para efeitos de divulgação da CONTRATADA, por parte da CONTRATADA. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.4 <b>Fica autorizada a CONTRATADA</b> posicionar sua logomarca, a título de identificação de sua autoria do (s) produto (s) e/ou serviço (s) que com ela forem contratados.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.5 Na hipótese de o CONTRATANTE ficar inadimplente com as obrigações financeiras, os produtos adquiridos por este contrato ficarão retidos até a resolução de tais pendências.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.6 AS PARTES, devidamente nomeadas e qualificadas no preâmbulo deste instrumento, responsabilizam-se solidariamente, em caráter irrevogável e irretratável, pelo cumprimento deste contrato.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>8.7 A obrigação ora reconhecida e assumida pelos CONTRATANTES, como líquida, certa e exigível, no valor  mencionado no <b>item 4.1</b>, aplica-se o disposto no artigo 784 ,III do Código de Processo Civil, vez que possui força de <b>Título Executivo Extrajudicial</b>.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>9 – DA PROTEÇÃO DE DADOS PESSOAIS</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1 A CONTRATADA, por si e por seus colaboradores, obriga- se a atuar no presente Contrato em conformidade com a Legislação vigente sobre Proteção de Dados Pessoais e as determinações de órgãos reguladores/fiscalizadores sobre a matéria, em especial a Lei 13.709/2018, além das demais normas e políticas de proteção de dados de cada país onde houver qualquer tipo de tratamento dos dados dos clientes, o que inclui os dados dos clientes desta. No manuseio dos dados a CONTRATADA deverá:</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.1 Tratar os dados pessoais a que tiver acesso apenas de acordo com as instruções da CONTRATANTE e em conformidade com estas cláusulas, e que, na eventualidade, de não mais poder cumprir estas obrigações, por qualquer razão, concorda em informar de modo formal este fato imediatamente à CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.2 Manter e utilizar medidas de segurança administrativas, técnicas e físicas apropriadas e suficientes para proteger a confidencialidade e integridade de todos os dados pessoais mantidos ou consultados/transmitidos eletronicamente, para garantir a proteção desses dados contra acesso não autorizado, destruição, uso, modificação, divulgação ou perda acidental ou indevida.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.3 Acessar os dados dentro de seu escopo e na medida abrangida por sua permissão de acesso (autorização) e que os dados pessoais não podem ser lidos, copiados, modificados ou removidos sem autorização expressa e por escrito da CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.4 Garantir, por si própria ou quaisquer de seus empregados, prepostos, sócios, diretores, representantes ou terceiros contratados, a confidencialidade dos dados processados, assegurando que todos estes que lidam com os dados pessoais sob responsabilidade da CONTRATANTE, assinaram Acordo de Confidencialidade com a CONTRATADA, bem como a manter quaisquer Dados Pessoais estritamente confidenciais e de não utilizá-los para outros fins, salvo para a fiel execução do objeto do contrato de prestação de serviço.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.5 A CONTRATADA em nenhuma hipótese repassará a terceiros informações relativas ao contrato de prestação de serviço firmado com a CONTRATANTE, salvo mediante requerimento expresso da CONTRATANTE, cuja autorização deverá, obrigatoriamente, conter o destinatário e os dados que serão objeto de compartilhamento. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>9.1.6 A CONTRATADA esclarece que os dados pessoais da CONTRATANTE ficarão em sua base de dados pelo período compreendido entre a data da assinatura do contrato, até 24 (vinte e quatro) meses após a entrega do produto/serviço contratado. Após essa data, todos os dados da CONTRATANTE serão destruídos/deletados do seu servidor; em caso de documento físico, estes serão, igualmente, destruídos.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>10 - DO FORO</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>10.1 Fica eleito o foro da Comarca da instituição de Ensino, com renúncia expressa de qualquer outro, por mais privilegiado que seja, para solucionar todas as questões divergentes, resultantes da interpretação deste instrumento e, para nele, serem demandados a execução do cumprimento de todas as obrigações oriundas deste contrato, não obstante qualquer mudança de domicílio.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>10.2 Por estarem assim, justos e acordados, assinam o presente instrumento em 02 (duas) vias de igual teor na presença de 02 (duas) testemunhas para que surta os efeitos jurídicos e legais.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>Goiânia, <?php echo date('d'); ?> de <?php $mes = date('m');
                                                                                if ($mes == 1) {
                                                                                    echo "Janeiro";
                                                                                }
                                                                                if ($mes == 2) {
                                                                                    echo "Fevereiro";
                                                                                }
                                                                                if ($mes == 3) {
                                                                                    echo "Março";
                                                                                }
                                                                                if ($mes == 4) {
                                                                                    echo "Abril";
                                                                                }
                                                                                if ($mes == 5) {
                                                                                    echo "Maio";
                                                                                }
                                                                                if ($mes == 6) {
                                                                                    echo "Junho";
                                                                                }
                                                                                if ($mes == 7) {
                                                                                    echo "Julho";
                                                                                }
                                                                                if ($mes == 8) {
                                                                                    echo "Agosto";
                                                                                }
                                                                                if ($mes == 9) {
                                                                                    echo "Setembro";
                                                                                }
                                                                                if ($mes == 10) {
                                                                                    echo "Outubro";
                                                                                }
                                                                                if ($mes == 11) {
                                                                                    echo "Novembro";
                                                                                }
                                                                                if ($mes == 12) {
                                                                                    echo "Dezembro";
                                                                                } ?> de <?php echo date('Y'); ?>.</p>
                                        </td>
                                        </tr>

                                        </table>


                                        <table width="100%" BORDER="1" style="border-collapse: collapse">
                                        <tr>


                                        <td  style="text-align: center; width: 50%;">

                                        <!--<img style="width: 50%;" src="../imgs/assinaturas/Assinatura_Lanucia_CONTRATADA.png" alt="">-->
                                        Lanúcia Evangelista da Costa Moraes
                                        <br>        
                                        Data Aceite: <?= $dataaceite ?> 
                                        <br>
                                        Hora Aceite: <?= $horaatual ?>
                                        <br>
                                        IP de Acesso: <?= gethostbyname('www.studiomfotografia.com.br')?>
                                        <p>_______________________________</p>
                                        CONTRATADA
                                        </td>


                                        <td style="text-align: center; width: 50%;">
                                        <p>
                                        <?= $vetor_cadastro['nome']; ?>
                                        <br> 
                                        <?= $formando['email']  ?>
                                        <br> 
                                        Data Aceite: <?= $dataaceite ?> 
                                        <br>
                                        Hora Aceite: <?= $horaatual ?>
                                        <br>
                                        IP de Acesso: <?= $ip ?>
                                        <p>_______________________________</p>
                                        CONTRATANTE
                                        </td>

                                        </tr>
                                            
                                        </table>

                                        <table width="100%" BORDER="1" style="border-collapse: collapse">
                                            <tr>
                                                <td style="text-align: center; width: 50%;">
                                                    <img style="width: 50%;" src="../imgs/assinaturas/Assinatura_Andressa_TESTEMUNHA1.png" alt="">
                                                    <p>_______________________________</p>
                                                    TESTEMUNHA 1                               
                                                </td>
                                                
                                                <td style="text-align: center; width: 50%;">
                                                    <img style="width: 50%;" src="../imgs/assinaturas/Assinatura_Carol_TESTEMUNHA2.png" alt="">
                                                    <p>_______________________________</p>
                                                    TESTEMUNHA 2
                                                </td>
                                            </tr>
                                        </table>
                                                                            </textarea>
                                                                                    </div>
                                                                                <?php } ?>
                                                                            </div>
                                                                            <input type="text" name="id_item" value="<?php echo $id_item; ?>" hidden>
                                                                            <input type="text" name="formapag" value="<?php echo $formapag; ?>" hidden>
                                                                            <input type="text" name="diavencimento" value="<?php echo $diavencimento; ?>"
                                                                                hidden>
                                                                            <input type="text" name="qtdparcelas" value="<?php echo $qtdparcelas; ?>" hidden>

                                                                            <input id="license-check" type="checkbox" value="2" required="" onclick='return pergunta(this);'/>
                                                                            <label for="license-check">Concordo com os termos apresentados neste contrato.</label>

                                                                            <input id="btn-next" class="btn btn-success" type="submit" value="Avançar"/>

                                </form>
                                <?php }?>       
                                <br>
                                <br>
                                <br>
                                <br>

                                <?php if ($pacote['tipoContrato'] == 4) {
                                    # Contrato Arquivo Digital - Ensaio Foto de Convite
                                ?>

                                <form method="post" action="compra_assinatura_fotografia.php?id=<?= $id_item?>">
                                    <div id="license-box">
                                        <?php if ($pacote_itens['pacote_especial'] == '3') { ?>
                                            <div class="text">
                                    <textarea id="mymce" disabled="true">
                                        <table width="100%">
                                            <tr>
                                                <td width="50%"><img src="../imgs/logo1.png" width="100px"></td>
                                                <td width="50%" align="center">(62)3218-3476
                                                    <br>
                                                    Rua 93 nº 296 qd. F-14 lt. 36
                                                    <br>
                                                    St. Sul, Goiânia-GO - CEP 74083-120
                                                    <br>
                                                    cliente@studiomfotografia.com.br
                                                </td>
                                            </tr>
                                        </table>
                                        <div>
                                        <p>Parabéns por sua aquisição <?php echo $vetor_cadastro['nome']; ?></p>
                                        <p>Você faz parte do contrato: <?php echo $vetor_turma['ncontrato'] . ' - ' . $vetor_curso['nome'] . ' ' . $vetor_instituicao['nome'] . ' - '. $vetor_instituicao['cidade'] . ' - ' . $vetor_instituicao['estado']; ?></p>

                                        <p>Este é o extrato de sua compra:</p>

                                        <table width="50%">
                                            <tr>
                                                <th><strong><h3>Produto</h3></strong></th>
                                                <th><strong><h3>Quantidade</h3></strong></th>
                                                <th><strong><h3>Sub-Total</h3></strong></th>
                                            </tr>
                                            <tr>
                                                <td align="center">Fotos para Convite</td>
                                                <td align="center">01 un.</td>
                                                <td align="center">R$<?php echo number_format($pacote_itens['valor'], 2, ',', '.'); ?></td>
                                            </tr>
                                            <tr>
                                                <td align="center"></td>
                                                <td align="center"></td>
                                                <td align="center">Total: R$<?php echo number_format($pacote_itens['valor'], 2, ',', '.'); ?></td>
                                            </tr>
                                        </table>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Sua forma de pagamento escolhida é: <?php echo $vetor_forma['nome']; ?> em <?php echo $qtdparcelas; ?> parcela(s).</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">O produto “Fotos para Convite” consiste especificamente nas fotografias a serem produzidas para o convite de formatura, exceto as fotos de Família. Tais exemplos de fotografias estão especificados no documento de “Orientação Fotográfica”, advindo do convite de formatura.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Após a realização das fotos para o convite, as mesmas serão disponibilizadas para a escolha através da plataforma digital da empresa para que possam receber o (s) devido (s) tratamento (s) indicados por você. O prazo para que isto aconteça é de até 45 dias úteis e/ou prazo estipulado entre a comissão de formatura e a Studio M Fotografia.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">A entrega destas fotos tratadas também será através da plataforma digital logo após o tratamento das imagens.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Ficará a cargo da comissão de formatura a solicitação do agendamento dos dias para a realização do evento “Ensaio Foto de Convite”, o qual faz parte as “Foto para Convite”.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">O associado que, por qualquer motivo, cancelar a compra ou não participar das “Fotos para o Convite” não terá direito a ressarcimento, pois o mesmo fará ou fez parte do planejamento de gastos das “Fotos para Convite”.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Declaro para todos os fins de direito que, efetuei a compra das “Fotos para Convite” e estou ciente que o valor cobrado por tal produto está presente no contrato de fotografia, o qual rege todas as regras da prestação do serviço.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Concordo e aprovo todos os compromissos que a comissão de formatura venha assumir em meu nome para a realização para as “Fotos para Convite”, acatando as decisões e medidas por ela tomadas, e, por ser verdade, dato e confirmo a a compra das “Fotos para Convite”.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Assim sendo, na condição de contratante, ciente das minhas obrigações contratuais, firmo meu ACEITE na compra do produto “Fotos para Convite” o qual é parte integrante do contrato de fotografia.</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">Aceite da compra feito por meio de confirmação via sistema recebido pelo link de confirmação:</p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">
                                            E-mail: <?php echo $vetor_cadastro['email']; ?> <br>
                                            Data Aceite: <br>
                                            Hora Aceite:  <br>
                                            IP de Acesso:  <br>
                                        </p>

                                        <p style="text-align: justify;" data-mce-style="text-align: justify;">
                                            Atenciosamente; <br>
                                            StudioM Fotografia
                                        </p>
                                    </textarea>
                                            </div>
                                        <?php }else { ?>
                                            <h1>Contrato Individual</h1>
                                            <div class="text">
                                    <textarea id="mymce2" disabled="true">
                                        <table width="100%">
                                            <tr>
                                                <td width="50%"><img src="data:image/png;base64," alt="" width="250px" /></td>
                                                <td width="50%">(62)3218-3476
                                                    <br>
                                        Rua 93 nº 296 qd. F-14 lt. 36
                                        <br>
                                        St. Sul, Goiânia-GO - CEP 74083-120
                                        <br>
                                        cliente@studiomfotografia.com.br
                                        </td>
                                            </tr>
                                        </table>
                                        <br>
                                        <div align="right">Contrato n° <?php echo $vetor_turma['ncontrato']; ?>></div>
                                        <p>
                                        </p>
                                        <p>
                                        </p>

                                        <p><strong><div align="center">CONTRATO INDIVIDUAL DE PRESTAÇÃO DE SERVIÇOS FOTOGRÁFICOS</div></strong></p>

                                        <p>
                                        </p>
                                        <table>

                                        <tr>
                                        <td>
                                        <p><strong>1 - PARTES CONTRATANTES</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>a) VENDEDORA CONTRATADA</strong>, doravante denominada simplesmente por Studio Med Fotografia e Turismo LTDA, pessoa jurídica de direito privado, inscrita no CNPJ sob o n° 24.917.717/0001-60, com sede na Rua 93 nº 296 qd. F-14 lt. 36, Cep: 74083-120, Setor Sul, Goiânia-Goiás.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>b) COMPRADOR CONTRATANTE</strong>, doravante designado apenas <strong>CONTRATANTE:</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>Nome:</strong> <?php echo $vetor_cadastro['nome']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>E-mail:</strong> <?php echo $vetor_cadastro['email']; ?> 
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>Endereço Completo:</strong> <?php echo $vetor_cadastro['endereco'] . '  ' .$vetor_cadastro['complemento']. '  ' . $vetor_cadastro['numero']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>Bairro:</strong> <?php echo $vetor_cadastro['bairro']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>Cidade:</strong> <?php echo $vetor_cadastro['cidade']; ?>   <strong>UF:</strong> <?php echo $vetor_cadastro['estado']; ?>   <strong>CEP:</strong> <?php echo $vetor_cadastro['cep']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>Telefone Resid.:</strong> <?php echo $vetor_cadastro['telefone']; ?>  <strong>Celular:</strong> <?php echo $vetor_cadastro['celular']; ?>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>CPF:</strong> <?php echo $vetor_cadastro['cpf']; ?> <strong>RG:</strong> <?php echo $vetor_cadastro['rg']; ?>    
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>2 - DO OBJETO DO CONTRATO</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">2.1 O presente instrumento de Contrato de Prestação de Serviços tem por objeto a comercialização do arquivo digital, nas condições abaixo estabelecidas.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">2.1.1 Compreende-se o produto <b>arquivo digital</b> o material fotográfico disponibilizado em mídia digital em alta resolução pela CONTRATADA ao CONTRATANTE. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>3 – DOS EVENTOS CONTRATADOS</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <table width="40%" class="table table-bordered table-striped">
                                                <?php
                                                $sql_eventos = mysqli_query($con, "select * from eventos_pacote WHERE id_pacote = '$pacote_itens[id_item]' order by id_evento_pacote ASC");
                                                $i = 0;
                                                while ($vetor_eventos = mysqli_fetch_array($sql_eventos)) {
                                                    $sql_evento = mysqli_query($con, "select * from eventos_turma_lista where id_evento_turma = '$vetor_eventos[id_evento]'");
                                                    $vetor_evento = mysqli_fetch_array($sql_evento);
                                                    $sql_evento_nome = mysqli_query($con, "select * from categoriaevento where id_categoria = '$vetor_evento[id_evento]'");
                                                    $vetor_evento_nome = mysqli_fetch_array($sql_evento_nome);
                                        ?>
                                                    <tr>
                                                    <td><?php if ($vetor_evento_nome['nome'] == 'Pré-evento'){$i++; echo $vetor_evento_nome['nome'].' '.$i; }else { echo $vetor_evento_nome['nome']; }?></td>
                                                    </tr>
                                                <?php
                                                }
                                                ?>


                                        </table>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>4 - DOS PRODUTOS ADQUIRIDOS E PREÇO</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <table width="100%" class="table table-bordered table-striped">
                                        <tr style="    background: #000000; color: #fff;">
                                        <td width="5%">Item</td>
                                        <td width="95%">Produtos</td>
                                        </tr>

                                                                                <?php
                                                                            $sql_itens = mysqli_query($con, "select * from pacotes_itens_produtos WHERE id_pacote = '{$pacote_itens['id_item']}' order by id_produto_item ASC");
                                        $i = 1;

                                        while ($vetor_item=mysqli_fetch_array($sql_itens)) {

                                        $sql_produto = mysqli_query($con, "select * from tipo_opcionais where id_tipo = '{$vetor_item['id_produto']}'");
                                        $vetor_produto = mysqli_fetch_array($sql_produto);

                                        $valorun = number_format($vetor_item['valor'],2,',','.');
                                        ?>
                                        <tr>
                                        <td><?php echo $i; ?></td>
                                        <td><?php echo $vetor_produto['nome']; ?>

                                                                                <?php
                                        if($vetor_produto['id_tipo'] == 10 || $vetor_produto['id_tipo'] == 11 || $vetor_produto['id_tipo'] == 12 || $vetor_produto['id_tipo'] == 13 || $vetor_produto['id_tipo'] == 15) {

                                        echo $vetor_item['qtdpaginas'].' páginas</td>';

                                        }
                                        $i++;
                                        }
                                        ?>
                                        </table>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">4.1 Em razão dos produtos adquiridos, especificamente, pelo CONTRATANTE, as partes têm por certo e ajustado que o valor total dos produtos contratados é de R$ <?php echo $num = number_format($valorfinal, 2, ',', '.'); ?>(<?php echo strtoupper(Monetary::numberToExt($valorfinal)); ?>), considerando a quantidade de páginas do álbum de fotografia provenientes do contrato geral com a comissão de formatura e produto (s) acima referenciado (s).</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <strong>5 - FORMAS DE PAGAMENTO</strong>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">5.1 O Pagamento será realizado em <?php $valorparcela = $valorfinal / $qtdparcelas;
                                                                                echo $qtdparcelas; ?> parcela (s) de R$<?php echo $num = number_format($valorparcela, 2, ',', '.'); ?> (<?php echo strtoupper(Monetary::numberToExt($valorparcela)); ?>),  através da seguinte opção:</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        • <?php echo $vetor_forma['nome']; ?>.
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>

                                        <table width="100%" class="table table-bordered table-striped">

                                        <tr style="    background: #000000; color: #fff;">
                                        <td width="20%">Parcela</td>
                                        <td width="20%">Valor</td>
                                        <td width="60%">Data de Vencimento</td>
                                        </tr>
                                                                                <?php
                                                                                $i = 1;
                                                                                while ($i <= $qtdparcelas) {
                                                                                if ($formapag != '8') {
                                                                                    if ($formapag == '18') {
                                                                                        $data = date('Y-m-d', strtotime('+4 days'));
                                                                                    }else {
                                                                                        $data = date('Y-m', strtotime('+'.$i.' months'));
                                                                                    }
                                                                                }else {
                                                                                    $data = date('Y-m', strtotime('+'.$i.' months', strtotime(substr($pacote['dataentrega'], 0, 7))));
                                                                                }
                                                                                $dataexplode = explode('-', $data);
                                                                                $mesdup = $dataexplode[1];
                                                                                if ($mesdup == 1) {
                                                                                    $mesgerado1 = "Janeiro";
                                                                                }
                                                                                if ($mesdup == 2) {
                                                                                    $mesgerado1 = "Fevereiro";
                                                                                }
                                                                                if ($mesdup == 3) {
                                                                                    $mesgerado1 = "Março";
                                                                                }
                                                                                if ($mesdup == 4) {
                                                                                    $mesgerado1 = "Abril";
                                                                                }
                                                                                if ($mesdup == 5) {
                                                                                    $mesgerado1 = "Maio";
                                                                                }
                                                                                if ($mesdup == 6) {
                                                                                    $mesgerado1 = "Junho";
                                                                                }
                                                                                if ($mesdup == 7) {
                                                                                    $mesgerado1 = "Julho";
                                                                                }
                                                                                if ($mesdup == 8) {
                                                                                    $mesgerado1 = "Agosto";
                                                                                }
                                                                                if ($mesdup == 9) {
                                                                                    $mesgerado1 = "Setembro";
                                                                                }
                                                                                if ($mesdup == 10) {
                                                                                    $mesgerado1 = "Outubro";
                                                                                }
                                                                                if ($mesdup == 11) {
                                                                                    $mesgerado1 = "Novembro";
                                                                                }
                                                                                if ($mesdup == 12) {
                                                                                    $mesgerado1 = "Dezembro";
                                                                                }
                                                                                $dataduplicata = $mesgerado1.'/'.$dataexplode[0];
                                                                                ?>
                                        <tr>
                                        <td><?php echo $i.'/'.$qtdparcelas; ?></td>
                                        <td>R$ <?php echo $num = number_format($valorparcela, 2, ',', '.'); ?></td>
                                        <td><?php echo $dataduplicata; ?></td>
                                        </tr>
                                        <?php
                                            $i++;
                                        }
                                        ?>
                                        </table>

                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>6 - DO INADIMPLEMENTO DAS PARCELAS E OBRIGAÇÕES</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;" >6.1 O não pagamento de qualquer parcela na data prevista caracterizará a mora da <b>CONTRATANTE</b> e, implicará no vencimento antecipado da dívida, independente de qualquer notificação do <b>CONTRATANTE</b>, com a incidência de multa de 2% (dois por cento), sobre a dívida original, acrescido de juros à taxa de 1% (um por cento) ao mês, bem como atualização monetária pelo INPC - Índice Nacional Preço ao Consumidor, além das despesas judiciais e dos honorários advocatórios aqui estabelecidos entre as partes em 20% (vinte por cento), a serem pagos pela <b>CONTRATANTE</b>. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">6.2 O presente instrumento poderá ser rescindido a qualquer tempo, desde que seja expressamente notificada a outra parte com 30 (trinta) dias de antecedência, sem prejuízo de aplicação das multas previstas nos itens seguintes. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">6.2.1 À parte (CONTRATANTE ou CONTRATADA) que rescindir voluntariamente o contrato, ficará sujeita a multa rescisória equivalente a 10% (dez por cento) sobre o valor dos eventos não realizados, listados no item 3 deste instrumento. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">6.2.2 Em caso de rescisão pela CONTRATANTE, essa ficará sujeita a restituir a CONTRATADA, toda e qualquer bonificação recebida durante a vigência do presente instrumento.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">6.3 As penalidades previstas nos itens <strong>6.2.1</strong> e <strong>6.2.2</strong> não afastará a obrigação da CONTRATANTE de efetuar o pagamento do (s) evento (s) executado (s), conforme item 3 deste instrumento.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">6.4 Para fim de aplicação da multa ajustada nos “itens <strong>6.2.1</strong> e <strong>6.2.2</strong>”, a parte deverá <strong>comunicar expressamente a outra</strong>, o descumprimento da eventual obrigação prevista neste instrumento, ocasião que, fica ajustado o prazo de 10 (dez) dias úteis para eventual esclarecimento ou, se for o caso, cumprimento da obrigação.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">6.5 <strong>A multa prevista no item 6.2.1</strong> não será aplicada ao CONTRATANTE que por ventura seja impedido de acompanhar a respectiva turma de formatura, em razão de transferência de instituição ou em caso de reprovação. Entretanto, ficará sujeito ao pagamento de todos os serviços prestados pela CONTRATADA e já entregue ao CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>7 - DA ENTREGA DO PRODUTO</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">7.1 É obrigação da CONTRATADA entregar os produtos adquiridos pelo CONTRATANTE dentro dos prazos estipulados e nas seguintes condições: </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">7.2 As fotos serão disponibilizadas para a escolha e aprovação do (s) produto (s)  <b>exclusivamente</b> pela plataforma digital (<span style="color: blue;text-decoration: underline">www.studiomfotografia.com.br</span> - área do formando); </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">7.3 Das Fotos dos Pré Eventos</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">7.3.1 <b>As imagens</b> captadas nos pré-eventos <b>serão disponibilizadas via site, da CONTRATADA</b> no período de 15 (quinze) a 30 (trinta) dias úteis, a contar do dia útil após a data do evento. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">7.4 Das Fotos dos Eventos Oficiais</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">7.4.1 Em até 45 dias úteis após a data do último evento da turma as fotos serão disponibilizadas para a escolha através da plataforma digital (<span style="color: blue;text-decoration: underline">www.studiomfotografia.com.br</span> - área do formando). </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">7.5 A CONTRATADA não se responsabilizará pelo não comparecimento do CONTRATANTE ao evento contratado. Levando em consideração que a prestação do serviço acontece de forma conjunta, será de inteira responsabilidade do CONTRATANTE INDIVIDUAL o seu comparecimento, e para tanto, em caso de ausência, não poderá ser abatido o valor correspondente a cobertura fotográfica desse evento.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>8 - DAS CONDIÇÕES GERAIS</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">8.1 As datas dos eventos extraoficiais e oficiais, incluindo a data para a realização das fotos para o convite de formatura serão informadas previamente pela comissão de formatura, não tendo o CONTRATANTE e CONTRATADA qualquer responsabilidade sobre o agendamento dos mencionados eventos. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">8.2 <b>A CONTRATANTE autoriza expressamente e sem qualquer ônus, a utilização das imagens</b> dos eventos de formatura, para efeitos de divulgação da CONTRATADA, por parte da CONTRATADA. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">8.3 <b>Fica autorizada a CONTRATADA</b> posicionar sua logomarca, a título de identificação de sua autoria do (s) produto (s) e/ou serviço (s) que com ela forem contratados.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">8.4 Na hipótese de o CONTRATANTE ficar inadimplente com as obrigações financeiras, os produtos adquiridos por este contrato ficarão retidos até a resolução de tais pendências.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">8.5 AS PARTES, devidamente nomeadas e qualificadas no preâmbulo deste instrumento, responsabilizam-se solidariamente, em caráter irrevogável e irretratável, pelo cumprimento deste contrato.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">8.6 A obrigação ora reconhecida e assumida pelos CONTRATANTES, como líquida, certa e exigível, no valor  mencionado no <b>item 4.1</b>, aplica-se o disposto no artigo 784 ,III do Código de Processo Civil, vez que possui força de <b>Título Executivo Extrajudicial</b>.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>9 – DA PROTEÇÃO DE DADOS PESSOAIS</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">9.1 A CONTRATADA, por si e por seus colaboradores, obriga- se a atuar no presente Contrato em conformidade com a Legislação vigente sobre Proteção de Dados Pessoais e as determinações de órgãos reguladores/fiscalizadores sobre a matéria, em especial a Lei 13.709/2018, além das demais normas e políticas de proteção de dados de cada país onde houver qualquer tipo de tratamento dos dados dos clientes, o que inclui os dados dos clientes desta. No manuseio dos dados a CONTRATADA deverá:</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">9.1.1 Tratar os dados pessoais a que tiver acesso apenas de acordo com as instruções da CONTRATANTE e em conformidade com estas cláusulas, e que, na eventualidade, de não mais poder cumprir estas obrigações, por qualquer razão, concorda em informar de modo formal este fato imediatamente à CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">9.1.2 Manter e utilizar medidas de segurança administrativas, técnicas e físicas apropriadas e suficientes para proteger a confidencialidade e integridade de todos os dados pessoais mantidos ou consultados/transmitidos eletronicamente, para garantir a proteção desses dados contra acesso não autorizado, destruição, uso, modificação, divulgação ou perda acidental ou indevida.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">9.1.3 Acessar os dados dentro de seu escopo e na medida abrangida por sua permissão de acesso (autorização) e que os dados pessoais não podem ser lidos, copiados, modificados ou removidos sem autorização expressa e por escrito da CONTRATANTE.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">9.1.4 Garantir, por si própria ou quaisquer de seus empregados, prepostos, sócios, diretores, representantes ou terceiros contratados, a confidencialidade dos dados processados, assegurando que todos estes que lidam com os dados pessoais sob responsabilidade da CONTRATANTE, assinaram Acordo de Confidencialidade com a CONTRATADA, bem como a manter quaisquer Dados Pessoais estritamente confidenciais e de não utilizá-los para outros fins, salvo para a fiel execução do objeto do contrato de prestação de serviço.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">9.1.5 A CONTRATADA em nenhuma hipótese repassará a terceiros informações relativas ao contrato de prestação de serviço firmado com a CONTRATANTE, salvo mediante requerimento expresso da CONTRATANTE, cuja autorização deverá, obrigatoriamente, conter o destinatário e os dados que serão objeto de compartilhamento. </p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">9.1.6 A CONTRATADA esclarece que os dados pessoais da CONTRATANTE ficarão em sua base de dados pelo período compreendido entre a data da assinatura do contrato, até 24 (vinte e quatro) meses após a entrega do produto/serviço contratado. Após essa data, todos os dados da CONTRATANTE serão destruídos/deletados do seu servidor; em caso de documento físico, estes serão, igualmente, destruídos.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p><strong>10 - DO FORO</strong></p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">10.1 Fica eleito o foro da Comarca da instituição de Ensino, com renúncia expressa de qualquer outro, por mais privilegiado que seja, para solucionar todas as questões divergentes, resultantes da interpretação deste instrumento e, para nele, serem demandados a execução do cumprimento de todas as obrigações oriundas deste contrato, não obstante qualquer mudança de domicílio.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p style="margin-left: 20px !important;">10.2 Por estarem assim, justos e acordados, assinam o presente instrumento em 02 (duas) vias de igual teor na presença de 02 (duas) testemunhas para que surta os efeitos jurídicos e legais.</p>
                                        </td>
                                        </tr>

                                        <tr>
                                        <td>
                                        <p>Goiânia, <?php echo date('d'); ?> de <?php $mes = date('m');
                                                                                if ($mes == 1) {
                                                                                    echo "Janeiro";
                                                                                }
                                                                                if ($mes == 2) {
                                                                                    echo "Fevereiro";
                                                                                }
                                                                                if ($mes == 3) {
                                                                                    echo "Março";
                                                                                }
                                                                                if ($mes == 4) {
                                                                                    echo "Abril";
                                                                                }
                                                                                if ($mes == 5) {
                                                                                    echo "Maio";
                                                                                }
                                                                                if ($mes == 6) {
                                                                                    echo "Junho";
                                                                                }
                                                                                if ($mes == 7) {
                                                                                    echo "Julho";
                                                                                }
                                                                                if ($mes == 8) {
                                                                                    echo "Agosto";
                                                                                }
                                                                                if ($mes == 9) {
                                                                                    echo "Setembro";
                                                                                }
                                                                                if ($mes == 10) {
                                                                                    echo "Outubro";
                                                                                }
                                                                                if ($mes == 11) {
                                                                                    echo "Novembro";
                                                                                }
                                                                                if ($mes == 12) {
                                                                                    echo "Dezembro";
                                                                                } ?> de <?php echo date('Y'); ?>.</p>
                                        </td>
                                        </tr>

                                        </table>


                                        <table width="100%" BORDER="1" style="border-collapse: collapse">
                                        <tr>


                                        <td  style="text-align: center; width: 50%;">

                                        <!--<img style="width: 50%;" src="../imgs/assinaturas/Assinatura_Lanucia_CONTRATADA.png" alt="">-->
                                        Lanúcia Evangelista da Costa Moraes
                                        <br>        
                                        Data Aceite: <?= $dataaceite ?> 
                                        <br>
                                        Hora Aceite: <?= $horaatual ?>
                                        <br>
                                        IP de Acesso: <?= gethostbyname('www.studiomfotografia.com.br')?>
                                        <p>_______________________________</p>
                                        CONTRATADA
                                        </td>


                                        <td style="text-align: center; width: 50%;">
                                        <p>
                                        <?= $vetor_cadastro['nome']; ?>
                                        <br> 
                                        <?= $formando['email']  ?>
                                        <br> 
                                        Data Aceite: <?= $dataaceite ?> 
                                        <br>
                                        Hora Aceite: <?= $horaatual ?>
                                        <br>
                                        IP de Acesso: <?= $ip ?>
                                        <p>_______________________________</p>
                                        CONTRATANTE
                                        </td>

                                        </tr>
                                            
                                        </table>

                                        <table width="100%" BORDER="1" style="border-collapse: collapse">
                                            <tr>
                                                <td style="text-align: center; width: 50%;">
                                                    <img style="width: 50%;" src="../imgs/assinaturas/Assinatura_Andressa_TESTEMUNHA1.png" alt="">
                                                    <p>_______________________________</p>
                                                    TESTEMUNHA 1                               
                                                </td>
                                                
                                                <td style="text-align: center; width: 50%;">
                                                    <img style="width: 50%;" src="../imgs/assinaturas/Assinatura_Carol_TESTEMUNHA2.png" alt="">
                                                    <p>_______________________________</p>
                                                    TESTEMUNHA 2
                                                </td>
                                            </tr>
                                        </table>
                                                                            </textarea>
                                                                                    </div>
                                                                                <?php } ?>
                                                                            </div>
                                                                            <input type="text" name="id_item" value="<?php echo $id_item; ?>" hidden>
                                                                            <input type="text" name="formapag" value="<?php echo $formapag; ?>" hidden>
                                                                            <input type="text" name="diavencimento" value="<?php echo $diavencimento; ?>"
                                                                                hidden>
                                                                            <input type="text" name="qtdparcelas" value="<?php echo $qtdparcelas; ?>" hidden>

                                                                            <input id="license-check" type="checkbox" value="2" required="" onclick='pergunta(this);'/>
                                                                            <label for="license-check">Concordo com os termos apresentados neste contrato.</label>

                                                                            <input id="btn-next" class="btn btn-success" type="submit" value="Avançar"/>

                                </form>
                                <?php }?>       
                                <br>
                                <br>
                                <br>
                                <br>

                            </div>
                        </div>
                    </div>
                </div>
                
                <!--Modal-->
                <div class="modal fade" id="modalTermo" tabindex="-1" role="dialog" aria-labelledby="modalTermo" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                        <div class="modal-content">
                            <div class="modal-header">
                                <p><strong><h5>Concordo com os termos apresentados.</h5></strong></p>   
                            </div>                                                   
                            <div class="modal-body">
                                <p>O Studio M declara a utilização dos Dados Pessoais com base no interesse legítimo ou execução de contrato para fins internos. Asseguramos que o Studio M está empenhado na proteção dos seus dados, em observância à legislação de proteção de dados. </p>
                                <p>O Studio M poderá compartilhar os Dados Pessoais quando necessário em decorrência de obrigação legal, determinação de autoridade competente ou decisão judicial, sendo certo que notificará os Titulares sobre a solicitação. O compartilhamento com as autoridades e órgãos reguladores poderá ter como objetivo, ainda, o auxílio em investigações e medidas de prevenção e combate a ilícitos, o exercício regular de direitos do Studio M ou o cumprimento de obrigação legal ou regulatória. Em todas as previsões, há o comprometimento de revelação somente das informações e Dados Pessoais estritamente necessários, limitando-se ao mínimo exigido.</p>
                                <p>Não manteremos seus dados pessoais por mais tempo do que o necessário para as finalidades declaradas acima, nem utilizaremos seus dados para outros fins, exceto nas hipóteses autorizadas por Lei.</p>
                                <p>Para exercer seu direito como titular dos dados pessoais, você pode entrar em contato com o Encarregado de Dados, através do e-mail: <a href="mailto:cliente@studiomfotografia.com.br">cliente@studiomfotografia.com.br</a></p>                                                       
                            </div>
                            <div class="modal-footer">
                                <button type="submit" class="btn btn-primary" onclick='marcar(); ' data-dismiss="modal">Concordar
                                </button>
                                <button type="button" class="btn btn-secondary" onclick='desmarcar();' data-dismiss="modal">Discordar
                                </button>                                                 
                            </div>
                        </div>
                        </div>
                </div>

            </div>
                                     
            <footer class="footer text-center">
                Todos direitos reservados. <a href="https://studiomfotografia.com.br">Studio M Fotografia</a>.
            </footer>
            <!-- ============================================================== -->
            <!-- End footer -->
            <!-- ============================================================== -->
        </div>
        <!-- ============================================================== -->
        <!-- End Page wrapper  -->
        <!-- ============================================================== -->
    </div>
    <!-- ============================================================== -->
    <!-- End Wrapper -->
    <!-- ============================================================== -->
    <!-- ============================================================== -->
    <!-- customizer Panel -->
    <!-- ============================================================== -->

    <div class="chat-windows"></div>
    <!-- ============================================================== -->
    <!-- All Jquery -->
    <!-- ============================================================== -->
    <!-- Bootstrap tether Core JavaScript -->
    <script>
        function pergunta(){ 
        // retorna true se confirmado, ou false se cancelado
        $('input[type="checkbox"]').on('change', function(e){
            if(e.target.checked){
                $('#modalTermo').modal();
                $('#license-check').prop('checked', false);
            }   
            
        });
            //var mensagem = new String("Concordo com os termos apresentados. \n\ \n\O Studio M declara a utilização dos Dados Pessoais com base no interesse legítimo ou execução de contrato para fins internos. Asseguramos que o Studio M está empenhado na proteção dos seus dados, em observância à legislação de proteção de dados. \n\ \n\O Studio M poderá compartilhar os Dados Pessoais quando necessário em decorrência de obrigação legal, determinação de autoridade competente ou decisão judicial, sendo certo que notificará os Titulares sobre a solicitação. O compartilhamento com as autoridades e órgãos reguladores poderá ter como objetivo, ainda, o auxílio em investigações e medidas de prevenção e combate a ilícitos, o exercício regular de direitos do Studio M ou o cumprimento de obrigação legal ou regulatória. Em todas as previsões, há o comprometimento de revelação somente das informações e Dados Pessoais estritamente necessários, limitando-se ao mínimo exigido. \n\ \n\Não manteremos seus dados pessoais por mais tempo do que o necessário para as finalidades declaradas acima, nem utilizaremos seus dados para outros fins, exceto nas hipóteses autorizadas por Lei. \n\ \n\Para exercer seu direito como titular dos dados pessoais, você pode entrar em contato com o Encarregado de Dados, através do e-mail: cliente@studiomfotografia.com.br");
            //return confirm(mensagem);
        }  
        
        function marcar(){
            $('#license-check').prop('checked', true); 
        }
        function desmarcar(){
            $('#license-check').prop('checked', false); 
        }
    </script>
    <script src="../layout/assets/libs/popper.js/dist/umd/popper.min.js"></script>
    <script src="../layout/assets/libs/bootstrap/dist/js/bootstrap.min.js"></script>
    <!-- apps -->
    <script src="../layout/dist/js/app.min.js"></script>
    <!-- minisidebar -->
    <script>
        $(function () {
            "use strict";
            $("#main-wrapper").AdminSettings({
                Theme: false, // this can be true or false ( true means dark and false means light ),
                Layout: 'vertical',
                LogoBg: 'skin5', // You can change the Value to be skin1/skin2/skin3/skin4/skin5/skin6
                NavbarBg: 'skin5', // You can change the Value to be skin1/skin2/skin3/skin4/skin5/skin6
                SidebarType: 'mini-sidebar', // You can change it full / mini-sidebar / iconbar / overlay
                SidebarColor: 'skin6', // You can change the Value to be skin1/skin2/skin3/skin4/skin5/skin6
                SidebarPosition: false, // it can be true / false ( true means Fixed and false means absolute )
                HeaderPosition: false, // it can be true / false ( true means Fixed and false means absolute )
                BoxedLayout: false, // it can be true / false ( true means Boxed and false means Fluid )
            });
        });
    </script>
    <script src="../layout/dist/js/app-style-switcher.js"></script>
    <!-- slimscrollbar scrollbar JavaScript -->
    <script src="../layout/assets/libs/perfect-scrollbar/dist/perfect-scrollbar.jquery.min.js"></script>
    <script src="../layout/assets/extra-libs/sparkline/sparkline.js"></script>
    <!--Wave Effects -->
    <script src="../layout/dist/js/waves.js"></script>
    <!--Menu sidebar -->
    <script src="../layout/dist/js/sidebarmenu.js"></script>
    <!--Custom JavaScript -->
    <script src="../layout/dist/js/custom.min.js"></script>
    <!--This page JavaScript -->
    <!--chartis chart-->
    </body>

    </html>
<?php } ?>