...
$id = $_GET['id'];
$sql_album = mysqli_query($con, "select * from meu_album where id_meualbum = '$id'");
$vetor_album = mysqli_fetch_array($sql_album);

$sql_produto_item_final = mysqli_query($con, "select * from tipo_opcionais where id_tipo = '$vetor_album[id_item]'");
$vetor_produto_item_final = mysqli_fetch_array($sql_produto_item_final);

$sql_escolha = mysqli_query($con, "select * from meu_album where id_formando = '$_SESSION[id_formando]'");
$vetor_escolha = mysqli_fetch_array($sql_escolha);

$total = mysqli_num_rows($sql_escolha);

$sql_escolha_finalizada = mysqli_query($con, "select * from meu_album where id_formando = '$_SESSION[id_formando]' and status IN ('3', '4')");
$vetor_escolha_finalizada = mysqli_fetch_array($sql_escolha_finalizada);

$total_finalizada = mysqli_num_rows($sql_escolha_finalizada);
                        ...
                        <div class="d-flex align-items-center">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">Aprovações - Produtos Fotografia</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title">Aprovações - Produtos Fotografia</h4>

                                <?php if($total == $total_finalizada) { ?>

                                <div class="alert alert-success" role="alert">
                                    <?php echo $vetor_cadastro['nome']; ?>, parabéns pela aprovação do seu <?php echo $vetor_produto_item_final['nome']; ?>. Logo entraremos em contato informando sobre o envio do (s) produto (s).
                                </div>

                                <?php } else { ?>

                                <div class="alert alert-danger" role="alert">
                                    <?php echo $vetor_cadastro['nome']; ?>, ainda restam produtos a serem aprovados. Por favor finalize a aprovação destes produtos. Deseja finalizar a aprovação para os demais produtos agora?.
                                </div>
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h4 class="modal-title" id="myModalLabel">Aprovações - Produtos Fotografia</h4>
                                                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                                            </div>
                                            <div class="modal-body">
                                                <p>Ainda restam produtos as quais as fotografias ainda precisam ser escolhidas. Informamos que seus produtos entrarão em nosso cronograma de produção apenas quando finalizar a escolha de todos eles, pois serão produzidos de uma só vez devido a logística da entrega..</p>
                                            </div>
                                            <div class="modal-footer">
                                                <a href="inicio.php"><button type="button" class="btn btn-danger"  style="    float: left;">Sair das Aprovações</button></a>
                                                <a href="consultaprodutosfotografia_continuar.php?id=<?php echo $id; ?>"><button type="button" class="btn btn-info" >Continuar nas Aprovações</button></a>
                                            </div>
                                        </div>
                                        <!-- /.modal-content -->
                                    </div>
                                ...
                                <table width="100%">
                                    <tr>
                                        <td width="3%"><button type="button" class="btn btn-primary" onclick="EscondeElemento('escondido')" value="Esconde Div"  style="    float: left;">Sim</button></td>
                                        <td width="1%"></td>
                                        <td><button type="button" class="btn btn-danger" data-toggle="modal" data-target="#myModal" data-whatever="@mdo"  style="    float: left;">Não</button></td>
                                    </tr>
                                </table>
                                ...
                                <div id="escondido">                                    
                                    ...
                                    <table id="lang_opt" class="table table-striped table-bordered display" style="width:100%">
                                        ...
                                        <tbody>
                                            ...
                                            $sql_atual = mysqli_query($con, "select * from meu_album where id_formando = '$_SESSION[id_formando]' and status IN ('1', '2') order by id_meualbum DESC");
                                            ...
                                            $sql_tipos = mysqli_query($con, "select * from tipo_opcionais where id_tipo = '$vetor[id_item]'");
                                            $vetor_tipos = mysqli_fetch_array($sql_tipos);
                                    
                                            ?>
                                            <tr>
                                                <td><?php echo $vetor_tipos['nome']; ?></td>
                                                <td><?php if($vetor['datafinal'] == NULL || $vetor['datafinal'] >= $dataatual) { ?><a href="inicioalbum.php?id=<?php echo $vetor['id_meualbum']; ?>"><button type="button" class="btn btn-default mesmo-tamanho" title="Ver ou Alterar Cadastro"><i class="fa fa-edit"></i></button></a><?php } else { ?><a href="veralbum.php?id=<?php echo $vetor['id_meualbum']; ?>"><button type="button" class="btn btn-default mesmo-tamanho" title="Ver ou Alterar Cadastro"><i class="fa fa-eye"></i></button></a><?php } ?></td>
                                            </tr>
                                            <?php } ?>
                                        </tbody>
                                    </table>
